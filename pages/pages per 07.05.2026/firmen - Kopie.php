...
function load_firma(mysqli $db, int $id): ?array {
  $st = $db->prepare("SELECT id,name,strasse,plz_ort,telefon,email,website,logo,bkp_id,bkp_ids FROM firmen WHERE id=?");
  $st->bind_param("i",$id);
  $st->execute();
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  try {
    ...
    if ($action === 'create') {
      ...
      $bkp_ids = isset($_POST['bkp_ids']) && is_array($_POST['bkp_ids']) ? implode(',', array_map('intval', $_POST['bkp_ids'])) : null;

      if ($name==='') throw new Exception("Name ist Pflichtfeld.");

      // 1) Insert
      $st = $mysqli->prepare("INSERT INTO firmen (name,strasse,plz_ort,telefon,email,website,logo,bkp_id,bkp_ids) VALUES (?,?,?,?,?,?,?,?,?)");
      $st->bind_param("sssssssis", $name,$strasse,$plz_ort,$telefon,$email,$website,$logo,$bkp_id,$bkp_ids);
      $st->execute(); 
      ...
      $st = $mysqli->prepare("UPDATE firmen SET name=?, strasse=?, plz_ort=?, telefon=?, email=?, website=?, logo=?, bkp_id=?, bkp_ids=? WHERE id=?");
      $st->bind_param("sssssssi si", $name,$strasse,$plz_ort,$telefon,$email,$website,$finalLogo,$bkp_id,$bkp_ids, $id);
      $st->execute(); $st->close();
...
<div class="container">
  ...
  <div class="card">
    ...
    <form method="post" autocomplete="on" enctype="multipart/form-data">
      ...
      <div class="grid">
        ...
        <label for="bkp_ids">Alle Gewerke (BKP)</label>
        <div style="display:flex; flex-direction:column; gap:8px;">
            <!-- Chip-Display -->
            <div id="bkp_chips_firmen" style="display:flex; flex-wrap:wrap; gap:6px; min-height:32px; padding:6px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px;"></div>
            <select id="bkp_ids" name="bkp_ids[]" class="form-input" multiple style="height:130px; font-size:13px; border-radius:10px;">
                <?php 
                $activeBkpIds = !empty($editing['bkp_ids']) ? explode(',', $editing['bkp_ids']) : [];
                foreach($bkpList as $b): ?>
                    <option value="<?= (int)$b['id'] ?>" <?= in_array((string)$b['id'], $activeBkpIds) ? 'selected' : '' ?>>
                        <?= htmlspecialchars($b['code'] . ' ' . $b['bezeichnung']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <small style="color:#64748b;">Klicken zum Auswählen · mit <b>Strg</b> für Mehrfachauswahl · Chip <b>×</b> zum Entfernen</small>
        </div>
  ...
  // BKP Chip Widget
  function initBkpChips(selectId, chipsId) {
    ...
    function renderChips() {
      ...
      selected.forEach(opt => {
        ...
        chip.className = 'bkp-chip';
        chip.innerHTML = `${opt.text} <button type="button" class="chip-remove" data-val="${opt.value}" title="Entfernen">×</button>`;
    ...
    renderChips(); // initial render on page load
  }

  document.addEventListener('DOMContentLoaded', function() {
    initBkpChips('bkp_ids', 'bkp_chips_firmen');
  });
