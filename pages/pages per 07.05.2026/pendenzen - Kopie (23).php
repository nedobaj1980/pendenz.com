        ...
        if ($isEditMode && $editPendenzId !== null) {
            ...
            if ($stmtUpdate) {
                $stmtUpdate->bind_param(
                    ...
                    $input['projekt_id'], $input['vorgangsart_id'], $input['objekt_id'], $input['wohnung_id'],
                    $finalTitel, $input['kurzbeschreibung'], $input['langbeschreibung'], $input['notiz'], $input['beschreibung'],
                    $input['status'], $wichtigkeit, $startdatum, $enddatum, $uhrzeit, $dauer,
                    $input['send_now'], $input['zustaendig_id'], $input['confirmation_required'],
                    $input['external_can_view'], $input['external_can_upload'], $input['public_enabled'],
                    $input['bkp_id'], $extraJson, $editPendenzId
                );
                $stmtUpdate->execute();
                $stmtUpdate->close();
            }
            ...
            if ($stmtInsert) {
                $stmtInsert->bind_param(
                    ...
                    $finalTitel, $input['kurzbeschreibung'], $input['langbeschreibung'], $input['notiz'], $input['beschreibung'],
                    $input['status'], $wichtigkeit, $startdatum, $enddatum, $uhrzeit, $dauer,
                    $input['send_now'], $currentUserId, $input['zustaendig_id'], $input['confirmation_required'],
                    $input['external_can_view'], $input['external_can_upload'], $input['public_enabled'],
                    $extraJson, $input['bkp_id']
                );
...
foreach (fetchAllAssoc($mysqli, $userSql) as $row) {
    ...
    if ((string)($row['firma_name']??'') !== '') { $label .= ' · ' . $row['firma_name']; }
    if ((string)($row['rolle']??'') !== '') { $label .= ' (' . $row['rolle'] . ')'; }
    $users[] = ['id' => $row['id'], 'name' => $row['name'], 'label' => $label];
}
$artMap = [];
foreach ($arten as $row) { $artMap[(int) $row['id']] = $row; }
...
foreach ($bkpKategorien as $row) {
    if ((int)$row['bkp_id'] > 0) { $bkpKategorienByBkp[(int)$row['bkp_id']][] = $row; }
    if ((int)$row['id'] > 0) { $bkpKategorieMap[(int)$row['id']] = $row; }
}
...
$mieterCatMap = []; foreach ($mieterKategorien as $row) { $mieterCatMap[(int) $row['id']] = $row; }
$mieterSubMap = []; foreach ($mieterSubkategorien as $row) { $mieterSubMap[(int) $row['id']] = $row; }
$vermieterCatMap = []; foreach ($vermieterKategorien as $row) { $vermieterCatMap[(int) $row['id']] = $row; }
$vermieterSubMap = []; foreach ($vermieterSubkategorien as $row) { $vermieterSubMap[(int) $row['id']] = $row; }

$mieterSubByCat = [];
foreach ($mieterSubkategorien as $row) { $mieterSubByCat[(int)$row['kategorie_id']][] = $row; }
$vermieterSubByCat = [];
foreach ($vermieterSubkategorien as $row) { $vermieterSubByCat[(int)$row['kategorie_id']][] = $row; }
...
$artMap = [];
foreach ($arten as $a) { $artMap[(int)$a['id']] = $a; }
...
<style>
    ...
    .recent-bar-scroller::-webkit-scrollbar { height: 4px; }
    .recent-bar-scroller::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
    ...
    .pendenzen-fab:hover {
        transform: scale(1.1) rotate(0deg); /* Neutral rotation on hover */
    ...
    .btn-quick-create .btn-icon-plus {
        color: #a855f7; /* Purple sign as in screenshot */
        font-size: 22px;
        font-weight: 400;
    }
...
<div class="pendenzen-page">
    <div class="pendenzen-wrap">
        ...
        <div class="pendenzen-dashboard">
            <div class="pendenzen-dashboard-top">
                ...
                <div class="pendenzen-dashboard-actions">
                    <a class="pendenzen-chip-link" id="btnQuickAdd" href="javascript:void(0)" onclick="openQuickModal()">⚡ Schnell-Erfassung</a>
        ...
        <div class="pendenzen-card" id="pendenzNeuContainer" style="margin-bottom:18px;">
            <button type="button" id="toggleForm" class="pendenzen-collapse-toggle" style="width:100%;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:16px 18px;background:#fff;border:0;cursor:pointer;">
                <span style="display:flex;align-items:center;gap:10px;">
                    <span class="toggle-icon" style="display:inline-flex;transition:transform .2s ease;">▾</span>
                    <span style="font-weight:800;color:var(--text-main);">Neue Pendenz</span>
                    <span id="formStatusPill" style="display:inline-flex;align-items:center;padding:4px 10px;border-radius:999px;background:#dcfce7;color:#166534;font-size:12px;font-weight:700;">Geöffnet</span>
                </span>
                <span style="font-size:12px;color:var(--text-muted);font-weight:700;">Separat eingebunden</span>
            </button>
            <div id="formContent" style="padding:0 18px 18px 18px;border-top:1px solid var(--border-color);">
                <iframe
                    id="pendenzNeuFrame"
...
<div class="pendenzen-quick-modal" id="quickDrawer">
    ...
    <div class="modal-body">
        <form id="quickCaptureForm" enctype="multipart/form-data">
            ...
            <div class="quick-grid-2">
                <div class="pendenzen-field">
                    ...
                    <select id="quickVorgang" name="vorgangsart_id" required>
                        ...
                        <?php foreach ($arten as $row): ?>
                            <option value="<?php echo (int) $row['id']; ?>" data-welt="<?php echo h($row['vorlagen_welt'] ?: 'bkp'); ?>">
                                <?php echo h($row['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="pendenzen-field">
                    ...
                    <select id="quickAssign" name="zustaendig_id" required>
                        ...
                        <?php foreach ($users ?? [] as $row): ?>
                            <option value="<?php echo (int)$row['id']; ?>"><?php echo h($row['label'] ?? $row['name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="quick-grid-2">
                <div class="pendenzen-field">
                    ...
                    <select id="quickWohnung" name="wohnung_id">
                        ...
                        <?php foreach ($wohnungen ?? [] as $row): ?>
                            <option value="<?php echo (int)$row['id']; ?>" data-objekt-id="<?php echo (int)($row['objekt_id']??0); ?>"><?php echo h($row['name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="pendenzen-field">
                    ...
                    <div style="display:flex; gap:5px">
                        <input type="date" id="quickDate" name="startdatum" value="<?php echo date('Y-m-d'); ?>" style="flex:1">
            ...
            <div id="quickTemplateArea" style="display:none; margin-top:15px; padding-top:15px; border-top:1px dashed #eee">
                    ...
                    <div class="pendenzen-field">
                        ...
                        <select id="qBkpId" name="bkp_id">
                            ...
                            <?php foreach ($bkpCodes as $row): ?>
                                <option value="<?php echo (int)$row['id']; ?>"><?php echo h($row['code'] . ' ' . $row['bezeichnung']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="pendenzen-field" style="margin-top:10px">
                        <label>Inhalt / Kategorie</label>
                        <select id="qBkpKatId" name="bkp_kategorie_id"><option value="">— wählen —</option></select>
                    </div>
                    <div class="pendenzen-field" style="margin-top:10px">
                        <label>Vorlage / Text</label>
                        <select id="qBkpTextId" name="bkp_text_id"><option value="">— wählen —</option></select>
                    </div>
                </div>
                <div id="qBlockMieter" style="display:none">
                    <div class="quick-grid-2">
                        ...
                        <div class="pendenzen-field">
                            ...
                            <select id="qMieterSubId" name="mieter_subkategorie_id"><option value="">— wählen —</option></select>
                        </div>
                    </div>
                </div>
                <div id="qBlockVermieter" style="display:none">
                    <div class="quick-grid-2">
                        ...
                        <div class="pendenzen-field">
                            <label>Subkategorie</label>
                            <select id="qVermieterSubId" name="vermieter_subkategorie_id"><option value="">— wählen —</option></select>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style="margin-top:15px">
                ...
                <div style="display:flex; gap:10px">
                    <label class="pendenzen-chip-link" style="background:#f1f5f9; color:#475569; border:1px solid #e2e8f0; cursor:pointer; flex:1; justify-content:center">
                        📸 Bilder wählen <input type="file" name="bilder[]" multiple accept="image/*" style="display:none">
                    </label>
                    <label class="pendenzen-chip-link" style="background:#f1f5f9; color:#475569; border:1px solid #e2e8f0; cursor:pointer; flex:1; justify-content:center">
