...
$children = [];
if ($root) {
    /** DB-Listing: Kinder unter $pathRel */
    function db_list_children_local(mysqli $db, int $pid, string $parentRel): array {
        if ($parentRel === '') {
            $sql = "SELECT name, rel_path, is_dir, size, DATE_FORMAT(mtime, '%d.%m.%Y %H:%i') AS mtime FROM fs_nodes WHERE project_id=? AND (parent_rel_path IS NULL OR parent_rel_path='') ORDER BY is_dir DESC, name ASC";
            $st = $db->prepare($sql);
            $st->bind_param("i", $pid);
        } else {
            $sql = "SELECT name, rel_path, is_dir, size, DATE_FORMAT(mtime, '%d.%m.%Y %H:%i') AS mtime FROM fs_nodes WHERE project_id=? AND parent_rel_path=? ORDER BY is_dir DESC, name ASC";
            $st = $db->prepare($sql);
    ...
    $children = db_list_children_local($mysqli, $projectId, $pathRel);
    if (!$children && function_exists('fs_list_children_live')) {
        $children = fs_list_children_live($root, $pathRel);
    }
}
