...
function pendenzFindFirstExistingColumn(mysqli $mysqli, string $table, array $candidates): ?string
{
    foreach ($candidates as $column) {
        if (pendenzColumnExists($mysqli, $table, $column)) {
            return $column;
        }
    }
    return null;
...
function pendenzFetchTemplateRows(mysqli $mysqli, string $table, array $map): array
{
    ...
    foreach ($map as $alias => $candidates) {
        $found = pendenzFindFirstExistingColumn($mysqli, $table, $candidates);
        if ($found !== null) {
            $columns[$alias] = $found;
        }
    }
    ...
    $orderParts = [];
    foreach (['code', 'name', 'title', 'short'] as $alias) {
        if (isset($columns[$alias])) {
            $orderParts[] = "`{$alias}` ASC";
        }
    }
    if (!$orderParts) {
        $orderParts[] = "`id` ASC";
    ...
    $sql = "SELECT " . implode(', ', $selectParts) . " FROM `{$table}` ORDER BY " . implode(', ', $orderParts);
...
function pendenzCombineTextParts(array $parts): string
{
    $clean = [];
...
$bkpVorlagen = pendenzFetchTemplateRows($mysqli, 'bkp_codes', [
    'id' => ['id'],
    'code' => ['bkp_code', 'code', 'nummer', 'position', 'bkp'],
    'name' => ['bezeichnung', 'name', 'titel_kategorie', 'kategorie'],
    'title' => ['titel', 'title'],
    'short' => ['beschreibung', 'kurzbeschreibung', 'description'],
]);

$mieterVorlagen = pendenzFetchTemplateRows($mysqli, 'pendenz_kategorien_mieter', [
    'id' => ['id'],
    'name' => ['titel_kategorie', 'kategorie', 'name', 'titel'],
    'title' => ['titel_kategorie', 'titel', 'title', 'name'],
    'short' => ['kurzbeschreibung', 'beschreibung', 'description'],
]);

$vermieterVorlagen = pendenzFetchTemplateRows($mysqli, 'pendenz_kategorien_vermieter', [
    'id' => ['id'],
    'name' => ['titel_kategorie', 'kategorie', 'name', 'titel'],
    'title' => ['titel_kategorie', 'titel', 'title', 'name'],
    'short' => ['kurzbeschreibung', 'beschreibung', 'description'],
...
$input = [
    ...
    'kurzbeschreibung' => '',
    'titel_frei' => '',
    'kurzbeschreibung_frei' => '',
    'bkp_vorlage_id' => null,
    'mieter_vorlage_id' => null,
    'vermieter_vorlage_id' => null,
    'langbeschreibung' => '',
    'beschreibung' => '',
    'notiz' => '',
    'status' => 'offen',
    ...
    if ($stmtEditLoad) {
        ...
        if ($editRow) {
            ...
            $input = [
                ...
                'titel_frei' => (string) ($extraData['titel_frei'] ?? ($editRow['titel'] ?? '')),
                'kurzbeschreibung_frei' => (string) ($extraData['kurzbeschreibung_frei'] ?? ($editRow['kurzbeschreibung'] ?? '')),
                'bkp_vorlage_id' => isset($extraData['bkp_vorlage_id']) && is_numeric((string) $extraData['bkp_vorlage_id']) ? (int) $extraData['bkp_vorlage_id'] : null,
                'mieter_vorlage_id' => isset($extraData['mieter_vorlage_id']) && is_numeric((string) $extraData['mieter_vorlage_id']) ? (int) $extraData['mieter_vorlage_id'] : null,
                'vermieter_vorlage_id' => isset($extraData['vermieter_vorlage_id']) && is_numeric((string) $extraData['vermieter_vorlage_id']) ? (int) $extraData['vermieter_vorlage_id'] : null,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && postStr('form_action') === 'set_cover') {
    ...
    $input['titel_frei'] = postStr('titel_frei');
    $input['kurzbeschreibung_frei'] = postStr('kurzbeschreibung_frei');
    $input['bkp_vorlage_id'] = postIntOrNull('bkp_vorlage_id');
    $input['mieter_vorlage_id'] = postIntOrNull('mieter_vorlage_id');
    $input['vermieter_vorlage_id'] = postIntOrNull('vermieter_vorlage_id');

    $bkpSelected = null;
    foreach ($bkpVorlagen as $row) {
        if ((int) ($row['id'] ?? 0) === (int) ($input['bkp_vorlage_id'] ?? 0)) {
            $bkpSelected = $row;
            break;
        }
    }
    ...
    $mieterSelected = null;
    foreach ($mieterVorlagen as $row) {
        if ((int) ($row['id'] ?? 0) === (int) ($input['mieter_vorlage_id'] ?? 0)) {
            $mieterSelected = $row;
            break;
        }
    }

    $vermieterSelected = null;
    foreach ($vermieterVorlagen as $row) {
        if ((int) ($row['id'] ?? 0) === (int) ($input['vermieter_vorlage_id'] ?? 0)) {
            $vermieterSelected = $row;
            break;
        }
    }

    $input['titel'] = pendenzCombineTextParts([
        $input['titel_frei'],
        $bkpSelected['title'] ?? '',
        $mieterSelected['title'] ?? '',
        $vermieterSelected['title'] ?? '',
    ]);

    $input['kurzbeschreibung'] = pendenzCombineTextParts([
        $input['kurzbeschreibung_frei'],
        $bkpSelected['short'] ?? '',
        $mieterSelected['short'] ?? '',
        $vermieterSelected['short'] ?? '',
    ]);
    ...
    if ($input['titel'] === '') {
        ...
        $extraJson = json_encode([
            ...
            'titel_frei' => $input['titel_frei'],
            'kurzbeschreibung_frei' => $input['kurzbeschreibung_frei'],
            'bkp_vorlage_id' => $input['bkp_vorlage_id'],
            'mieter_vorlage_id' => $input['mieter_vorlage_id'],
            'vermieter_vorlage_id' => $input['vermieter_vorlage_id'],
        ...
        <div class="pendenzen-card">
            ...
            <div class="pendenzen-content" id="formContent" style="display: none;">
                ...
                <form method="post" action="" enctype="multipart/form-data" class="pendenzen-form" id="pendenzenForm">
                    ...
                    <!-- Sektion 2: Titel / Kurzbeschreibung -->
                    <div class="form-section">
                        <div class="form-section-head">🧩 Titel & Kurzbeschreibung</div>

                        <div class="pendenzen-grid-2">
                            <div>
                                <div class="pendenzen-grid-2" style="margin-bottom:20px;">
                                    <div class="pendenzen-field">
                                        <label for="titel_frei">Leerer Titel</label>
                                        <input type="text" name="titel_frei" id="titel_frei" value="<?php echo h($input['titel_frei']); ?>">
                                    </div>

                                    <div class="pendenzen-field">
                                        <label for="kurzbeschreibung_frei">Leere Kurzbeschreibung</label>
                                        <input type="text" name="kurzbeschreibung_frei" id="kurzbeschreibung_frei" value="<?php echo h($input['kurzbeschreibung_frei']); ?>">
                                    </div>
                                </div>

                                <div class="pendenzen-grid-2 pendenz-template-block" id="block_bkp" style="margin-bottom:16px;">
                                    <div class="pendenzen-field">
                                        <label for="bkp_vorlage_id">BKP Titel</label>
                                        <select name="bkp_vorlage_id" id="bkp_vorlage_id">
                                            <option value="">— auswählen —</option>
                                            <?php foreach ($bkpVorlagen as $row): ?>
                                                <?php $label = trim(((string) ($row['code'] ?? '')) . ' ' . ((string) ($row['name'] ?? ''))); ?>
                                                <option value="<?php echo (int) $row['id']; ?>"
                                                    data-title="<?php echo h((string) ($row['title'] ?? '')); ?>"
                                                    data-short="<?php echo h((string) ($row['short'] ?? '')); ?>"
                                                    <?php echo ((int) ($input['bkp_vorlage_id'] ?? 0) === (int) $row['id']) ? 'selected' : ''; ?>>
                                                    <?php echo h($label !== '' ? $label : ((string) ($row['title'] ?? 'BKP'))); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="pendenzen-field">
                                        <label for="bkp_beschreibung_preview">BKP Beschreibung</label>
                                        <input type="text" id="bkp_beschreibung_preview" value="<?php
                                            $selShort = '';
                                            foreach ($bkpVorlagen as $row) {
                                                if ((int)($input['bkp_vorlage_id'] ?? 0) === (int)$row['id']) { $selShort = (string)($row['short'] ?? ''); break; }
                                            }
                                            echo h($selShort);
                                        ?>" readonly>
                                    </div>
                                ...
                                <div class="pendenzen-grid-2 pendenz-template-block" id="block_mieter" style="margin-bottom:16px;">
                                    <div class="pendenzen-field">
                                        <label for="mieter_vorlage_id">Mieter Titel</label>
                                        <select name="mieter_vorlage_id" id="mieter_vorlage_id">
                                            <option value="">— auswählen —</option>
                                            <?php foreach ($mieterVorlagen as $row): ?>
                                                <option value="<?php echo (int) $row['id']; ?>"
                                                    data-title="<?php echo h((string) ($row['title'] ?? '')); ?>"
                                                    data-short="<?php echo h((string) ($row['short'] ?? '')); ?>"
                                                    <?php echo ((int) ($input['mieter_vorlage_id'] ?? 0) === (int) $row['id']) ? 'selected' : ''; ?>>
                                                    <?php echo h((string) (($row['name'] ?? '') !== '' ? $row['name'] : ($row['title'] ?? 'Mieter'))); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="pendenzen-field">
                                        <label for="mieter_beschreibung_preview">Mieter Beschreibung</label>
                                        <input type="text" id="mieter_beschreibung_preview" value="<?php
                                            $selShort = '';
                                            foreach ($mieterVorlagen as $row) {
                                                if ((int)($input['mieter_vorlage_id'] ?? 0) === (int)$row['id']) { $selShort = (string)($row['short'] ?? ''); break; }
                                            }
                                            echo h($selShort);
                                        ?>" readonly>
                                    </div>
                                ...
                                <div class="pendenzen-grid-2 pendenz-template-block" id="block_vermieter">
                                    <div class="pendenzen-field">
                                        <label for="vermieter_vorlage_id">Vermieter Titel</label>
                                        <select name="vermieter_vorlage_id" id="vermieter_vorlage_id">
                                            <option value="">— auswählen —</option>
                                            <?php foreach ($vermieterVorlagen as $row): ?>
                                                <option value="<?php echo (int) $row['id']; ?>"
                                                    data-title="<?php echo h((string) ($row['title'] ?? '')); ?>"
                                                    data-short="<?php echo h((string) ($row['short'] ?? '')); ?>"
                                                    <?php echo ((int) ($input['vermieter_vorlage_id'] ?? 0) === (int) $row['id']) ? 'selected' : ''; ?>>
                                                    <?php echo h((string) (($row['name'] ?? '') !== '' ? $row['name'] : ($row['title'] ?? 'Vermieter'))); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="pendenzen-field">
                                        <label for="vermieter_beschreibung_preview">Vermieter Beschreibung</label>
                                        <input type="text" id="vermieter_beschreibung_preview" value="<?php
                                            $selShort = '';
                                            foreach ($vermieterVorlagen as $row) {
                                                if ((int)($input['vermieter_vorlage_id'] ?? 0) === (int)$row['id']) { $selShort = (string)($row['short'] ?? ''); break; }
                                            }
                                            echo h($selShort);
                                        ?>" readonly>
                                    </div>
                            ...
                            <div>
                                <div class="pendenzen-field" style="margin-bottom:16px;">
                        ...
                        <div class="pendenzen-grid-2" style="margin-top:20px;">
                            <div class="pendenzen-field">
                                <label for="titel">Titel (Ergebnis)</label>
                                <input type="text" name="titel" id="titel" value="<?php echo h($input['titel']); ?>" required readonly>
                            </div>

                            <div class="pendenzen-field">
                                <label for="kurzbeschreibung">Kurzbeschreibung (Ergebnis)</label>
                                <input type="text" name="kurzbeschreibung" id="kurzbeschreibung" value="<?php echo h($input['kurzbeschreibung']); ?>" readonly>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Sektion 3: Zeit -->
                    <div class="form-section">
                        <div class="form-section-head">🕒 Zeit</div>
                        <div class="pendenzen-grid" style="margin-top:4px;">
                            <div class="pendenzen-field">
...
<script>
    (function () {
        ...
        const titelFrei = document.getElementById('titel_frei');
        const kurzbeschreibungFrei = document.getElementById('kurzbeschreibung_frei');
        const titelResult = document.getElementById('titel');
        const kurzbeschreibungResult = document.getElementById('kurzbeschreibung');
        const bkpVorlage = document.getElementById('bkp_vorlage_id');
        const mieterVorlage = document.getElementById('mieter_vorlage_id');
        const vermieterVorlage = document.getElementById('vermieter_vorlage_id');
        ...
        const bkpBeschreibungPreview = document.getElementById('bkp_beschreibung_preview');
        const mieterBeschreibungPreview = document.getElementById('mieter_beschreibung_preview');
        const vermieterBeschreibungPreview = document.getElementById('vermieter_beschreibung_preview');

        function readTemplateData(selectEl) {
            if (!selectEl) {
                return { title: '', short: '' };
            }
            ...
            return {
                title: (opt.getAttribute('data-title') || '').trim(),
                short: (opt.getAttribute('data-short') || '').trim()
            };
        }
        ...
        function currentVorgangText() {
            const opt = vorgangsart.options[vorgangsart.selectedIndex];
            return opt ? String(opt.textContent || '').toLowerCase() : '';
        }

        function updateTemplateVisibility() {
            const text = currentVorgangText();
            const isUnternehmer = text.includes('unternehmer');
            const isMieter = text.includes('mieter') && !text.includes('vermieter');
            const isVermieter = text.includes('vermieter');

            if (blockBkp) {
                blockBkp.style.display = isUnternehmer || (!isMieter && !isVermieter) ? '' : 'none';
                if (blockBkp.style.display === 'none' && bkpVorlage) {
                    bkpVorlage.value = '';
                }
            }
            if (blockMieter) {
                blockMieter.style.display = isMieter || (!isUnternehmer && !isVermieter) ? '' : 'none';
                if (blockMieter.style.display === 'none' && mieterVorlage) {
                    mieterVorlage.value = '';
                }
            }
            if (blockVermieter) {
                blockVermieter.style.display = isVermieter || (!isUnternehmer && !isMieter) ? '' : 'none';
                if (blockVermieter.style.display === 'none' && vermieterVorlage) {
                    vermieterVorlage.value = '';
                }
            }
        }

        function updateCombinedTemplateFields() {
            const bkpData = readTemplateData(bkpVorlage);
            const mieterData = readTemplateData(mieterVorlage);
            const vermieterData = readTemplateData(vermieterVorlage);

            if (titelResult) {
                titelResult.value = combineUnique([
                    titelFrei ? titelFrei.value : '',
                    bkpData.title,
                    mieterData.title,
                    vermieterData.title
                ]);
            ...
            if (kurzbeschreibungResult) {
                kurzbeschreibungResult.value = combineUnique([
                    kurzbeschreibungFrei ? kurzbeschreibungFrei.value : '',
                    bkpData.short,
                    mieterData.short,
                    vermieterData.short
                ]);
        ...
        [titelFrei, kurzbeschreibungFrei, bkpVorlage, mieterVorlage, vermieterVorlage].forEach(function (el) {
            if (el) {
                el.addEventListener('input', updateCombinedTemplateFields);
                el.addEventListener('change', updateCombinedTemplateFields);
            }
        });
