...
ini_set('display_errors', 0); // Sicherheit: Fehler nicht öffentlich zeigen
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '')==='confirm_done') {
  ...
  // Strikte Slot-Definition zur Vermeidung von Massen-Uploads
  $uploadMap = [
    'unt_img1' => ['type'=>'image', 'title'=>'Unternehmer Bild 1', 'file'=>'unt_1.jpg'],
    'unt_img2' => ['type'=>'image', 'title'=>'Unternehmer Bild 2', 'file'=>'unt_2.jpg'],
    'unt_pdf'  => ['type'=>'file',  'title'=>'Unternehmer Dokument', 'file'=>'unt_doc.pdf']
  ];
  ...
  foreach ($uploadMap as $key => $info) {
    if (!empty($_FILES[$key]['name']) && $_FILES[$key]['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $allowed, true)) {
           // 1. Alten Datenbank-Eintrag für diesen Slot löschen
           ...
           // 2. Datei physisch speichern (fixer Name verhindert Datei-Flut)
           $destDir = __DIR__ . '/../uploads/pendenzen/' . (int)$p['id'] . '/unternehmer';
           if (!is_dir($destDir)) @mkdir($destDir, 0777, true);
           
           $destPath = $destDir . '/' . $info['file'];
           if (move_uploaded_file($f['tmp_name'], $destPath)) {
              ...
              if ($info['type']==='image' && function_exists('imagecreatefromjpeg')) {
                  ...
                  if ($img) {
                      ...
                      imagejpeg($img, $destPath, 85); imagedestroy($img);
                  }
              }
              // 3. Neuen DB-Eintrag erstellen
              $dbPath = 'uploads/pendenzen/' . (int)$p['id'] . '/unternehmer/' . $info['file'];
  ...
  $st->execute();
  $flash="✅ Rückmeldung erfolgreich gespeichert.";
...
$untFiles = [];
$res = $mysqli->query("SELECT * FROM pendenz_dateien WHERE pendenz_id=".(int)$p['id']." AND (titel='Unternehmer Bild 1' OR titel='Unternehmer Bild 2' OR titel='Unternehmer Dokument')");
...
<head>
  ...
  <style>
    ...
    .upload-row { margin-bottom:15px; padding:12px; background:#f8fafc; border-radius:8px; border:1px solid #e2e8f0; }
    .upload-row label { font-weight:bold; font-size:14px; color:#475569; display:block; margin-bottom:5px; }
    .existing-file { font-size:12px; color:#10b981; margin-top:5px; display:flex; align-items:center; gap:5px; }
