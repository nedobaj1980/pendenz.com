...
if (!function_exists('combineUniqueTexts')) {
    function combineUniqueTexts(string ...$parts): string
    {
        $clean = [];
        foreach ($parts as $part) {
            ...
            foreach ($clean as $existing) {
                if (mb_strtolower($existing, 'UTF-8') === mb_strtolower($part, 'UTF-8')) {
                    $exists = true;
                    break;
                }
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $input['titel'] = $input['titel_manuell'];
    $input['kurzbeschreibung'] = $input['kurzbeschreibung_manuell'];
    ...
    $autoTitel = combineUniqueTexts($selectedKategorieName, $selectedSubName);
    $autoKurz = trim($selectedSubKurz !== '' ? $selectedSubKurz : $selectedSubName);

    $input['titel'] = combineUniqueTexts($input['titel_manuell'], $autoTitel);
    $input['kurzbeschreibung'] = combineUniqueTexts($input['kurzbeschreibung_manuell'], $autoKurz);
...
foreach ($kategorien as $row) {
    if ((int) ($row['id'] ?? 0) === (int) ($input['kategorie_id'] ?? 0)) {
        $renderAutoTitel = trim((string) ($row['name'] ?? ''));
        break;
    }
}

foreach ($subkategorien as $row) {
    if ((int) ($row['id'] ?? 0) === (int) ($input['subkategorie_id'] ?? 0)) {
        $subName = trim((string) ($row['name'] ?? ''));
        $subKurz = trim((string) ($row['beschreibung'] ?? $subName));
        $renderAutoTitel = combineUniqueTexts($renderAutoTitel, $subName);
        $renderAutoKurz = $subKurz !== '' ? $subKurz : $subName;
        break;
    }
}
?>
            ...
            <div class="pendenzen-content" id="formContent" style="display: none;">
                ...
                <form method="post" action="" class="pendenzen-form" id="pendenzenForm">
                    ...
                    <div class="form-section">
                        ...
                        <div class="pendenzen-grid">
                            <div class="pendenzen-field">
                                <label for="titel_manuell">Titel</label>
                                <input type="text" name="titel_manuell" id="titel_manuell" value="<?php echo h($input['titel_manuell']); ?>" placeholder="Eigener Titel">
                                <input type="hidden" name="titel" id="titel" value="<?php echo h($input['titel']); ?>">
                            </div>

                            <div class="pendenzen-field">
                                <label for="kurzbeschreibung_manuell">Kurzbeschreibung</label>
                                <input type="text" name="kurzbeschreibung_manuell" id="kurzbeschreibung_manuell" value="<?php echo h($input['kurzbeschreibung_manuell']); ?>" placeholder="Eigene Kurzbeschreibung">
                                <input type="hidden" name="kurzbeschreibung" id="kurzbeschreibung" value="<?php echo h($input['kurzbeschreibung']); ?>">
                            </div>
                        </div>
                        ...
                        <div class="pendenzen-grid" style="margin-top:20px;">
                            <div class="pendenzen-field">
                                <label for="titel_vorlage">Vorlagen-Titel</label>
                                <input type="text" id="titel_vorlage" value="<?php echo h($renderAutoTitel); ?>" placeholder="Wird aus Kategorie und Auswahl übernommen" readonly>
                            </div>

                            <div class="pendenzen-field">
                                <label for="kurzbeschreibung_vorlage">Vorlagen-Kurzbeschreibung</label>
                                <input type="text" id="kurzbeschreibung_vorlage" value="<?php echo h($renderAutoKurz); ?>" placeholder="Wird aus der Auswahl übernommen" readonly>
                            </div>
                        </div>
                ...
                <div class="pendenzen-list">
                    ...
                    <div id="tableSettingsPanel" class="form-section"
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 8px 15px; margin-top: 10px;"
                            ...
                            <label><input type="checkbox" data-col="24" checked> Aktion</label>
                        </div>
                    </div>
                    ...
                    <div class="pendenzen-table-wrap">
                        <table id="pendenzenTable" class="pendenzen-table">
                            ...
                            <tbody>
                                <?php if (!$recent): ?>
                                    <tr>
                                        <td colspan="25" class="pendenzen-table-empty">Keine Pendenzen gefunden.</td>
                                    </tr>
                                <?php else: ?>
...
<script>
    (function () {
        ...
        const titelManuellInput = document.getElementById('titel_manuell');
        const kurzbeschreibungManuellInput = document.getElementById('kurzbeschreibung_manuell');
        const titelVorlageInput = document.getElementById('titel_vorlage');
        const kurzbeschreibungVorlageInput = document.getElementById('kurzbeschreibung_vorlage');
        ...
        const form = document.getElementById('pendenzenForm');
        ...
        function joinUniqueTexts(parts) {
            ...
            parts.forEach((part) => {
                ...
                const exists = clean.some((existing) => existing.toLowerCase() === value.toLowerCase());
                if (!exists) {
                    clean.push(value);
                }
            });
        ...
        function updateTemplateDrivenFields() {
            ...
            if (titelVorlageInput) {
                titelVorlageInput.value = auto.title;
            }
            if (kurzbeschreibungVorlageInput) {
                kurzbeschreibungVorlageInput.value = auto.kurz;
            }

            if (titelInput) {
                titelInput.value = joinUniqueTexts([
                    titelManuellInput ? titelManuellInput.value : '',
                    auto.title
                ]);
            }

            if (kurzbeschreibungInput) {
                kurzbeschreibungInput.value = joinUniqueTexts([
                    kurzbeschreibungManuellInput ? kurzbeschreibungManuellInput.value : '',
                    auto.kurz
                ]);
            }
        }
        ...
        if (titelManuellInput) {
            titelManuellInput.addEventListener('input', updateTemplateDrivenFields);
        }

        if (kurzbeschreibungManuellInput) {
            kurzbeschreibungManuellInput.addEventListener('input', updateTemplateDrivenFields);
        }
