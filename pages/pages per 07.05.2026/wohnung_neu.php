...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $mysqli->prepare("
            INSERT INTO wohnungen (objekt_id, name, etage, flaeche, zimmer, badezimmer, balkon, wintergarten, letzter_renovation, gesamtzustand)
            VALUES (?,?,?,?,?,?,?,?,?,?)
        ");
        $stmt->bind_param(
            "issddiiiss",
            $objekt_id, $name, $etage, $flaeche, $zimmer, $badezimmer, $balkon, $wintergarten, $letzte_ren, $zustand
        );
        $stmt->execute();
        $wid = $stmt->insert_id;
        $stmt->close();
