...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_node') {
        ...
        if ($folder) {
            $mysqli->query("INSERT INTO ordner_vorlagen_nodes (vorlage_id, name, rel_path) VALUES ($tplId, '".$mysqli->real_escape_string($folder)."', '".$mysqli->real_escape_string($folder)."')");
...
<div class="container" style="max-width:1400px; padding-top: 20px;">
  ...
  <div class="tpl-grid">
     <?php while($tpl = $templates->fetch_assoc()): 
         $tplId = $tpl['id'];
         $nodes = $mysqli->query("SELECT * FROM ordner_vorlagen_nodes WHERE vorlage_id=$tplId ORDER BY name");
     ?>
        <div class="tpl-card">
