...
if (isset($_GET['ajax_hierarchy'])) {
  ...
  if ($pId > 0) {
    $q = $mysqli->query("SELECT id, name FROM objekte WHERE projekt_id=$pId ORDER BY name");
    while ($r = $q->fetch_assoc())
      $res['objects'][] = $r;
  }
  if ($oId > 0) {
    $q = $mysqli->query("SELECT id, name FROM wohnungen WHERE objekt_id=$oId ORDER BY name");
    while ($r = $q->fetch_assoc())
      $res['units'][] = $r;
  }
...
if (is_file($__utPath)) {
  ...
  if (function_exists('app_log'))
    app_log('Hinweis: includes/user_taxonomy.php fehlt – Typ/Status-UI deaktiviert.');
}
...
foreach ($objectsAll as $o) {
  if (!isset($hierarchyJS['objects'][$o['projekt_id']]))
    $hierarchyJS['objects'][$o['projekt_id']] = [];
  $hierarchyJS['objects'][$o['projekt_id']][] = $o;
}
foreach ($unitsAll as $u) {
  if (!isset($hierarchyJS['units'][$u['objekt_id']]))
    $hierarchyJS['units'][$u['objekt_id']] = [];
...
if ($preW > 0) {
  ...
  if ($rowH) {
    $preO = (int) $rowH['objekt_id'];
    $preP = (int) $rowH['projekt_id'];
  }
}
...
foreach (array_keys($FIELDS) as $k) {
  if (empty($profileVis[$k]))
    $profileVis[$k] = 'private';
}
foreach (['profilbild', 'firmenlogo', 'titelbild'] as $imgK) {
  $key = 'vis_' . $imgK;
  if (empty($profileVis[$key]))
    $profileVis[$key] = 'private';
}
if (!isset($profileVis['titelbild_max_height']))
  $profileVis['titelbild_max_height'] = 260;
if (!isset($profileVis['profilbild_max_size']))
  $profileVis['profilbild_max_size'] = 160;
if (!isset($profileVis['firmenlogo_max_size']))
  $profileVis['firmenlogo_max_size'] = 160;
...
function sanitize_image_url(?string $url, string $PREFIX): ?string
{
  ...
  if (strpos($low, 'data:image/svg+xml') === 0)
    return null;
...
function shrink_image_under_limit(string $pathOrUrl, int $maxW, int $maxH, int $maxBytes, string $PREFIX): ?string
{
  if (!$pathOrUrl)
    return null;
  $rel = _bp_prefix_strip($pathOrUrl, $PREFIX);
  if (!preg_match('~^uploads/~', ltrim($rel, '/')))
    return $pathOrUrl;
  $abs = _bp_rel_to_abs($rel);
  if (!file_exists($abs))
    return $pathOrUrl;
  ...
  switch ($mime) {
    ...
    case 'image/png':
      ...
      break;
    case 'image/gif':
      $im = @imagecreatefromgif($abs);
      break;
    default:
      $im = @imagecreatefromstring(@file_get_contents($abs));
      $mime = 'image/jpeg';
  }
  if (!$im)
    return $pathOrUrl;
...
function audience_can_see($fieldLevel, $audience)
{
  switch ($audience) {
    case 'internal':
      return in_array($fieldLevel, ['internal', 'public'], true);
    case 'project':
      return in_array($fieldLevel, ['project', 'public'], true);
    case 'public':
      return $fieldLevel === 'public';
    default:
      return true; // self
  }
}
    ...
    <?php if ($titelURL && $showImg('vis_titelbild')): ?>
      <div class="top-title">
        <img src="<?= h($titelURL) ?>" alt="Titelbild"
          style="width:100%;max-height:<?= (int) $tbMaxH ?>px;object-fit:cover;">
      </div>
    <?php endif; ?>
    ...
    <div class="summary">
      ...
      <div>
        <?php foreach ($FIELDS as $k => $label):
          ...
          if ($val === '' && $k !== 'name')
            continue;
          if (!$isSelf && !audience_can_see($profileVis[$k] ?? 'private', $aud))
            continue; ?>
          <div class="kv">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if ($action === 'invite_user') {
      if (!$canViewOthers)
        throw new Exception("Keine Berechtigung, Einladungen zu versenden.");
      $uid = (int) ($_POST['id'] ?? 0);
      if ($uid <= 0)
    ...
    /* === Ordner Sync === */ elseif ($action === 'sync_folder') {
      $uid = (int) ($_POST['id'] ?? 0);
      $res = user_folder_sync($mysqli, $uid);
      $flash = ($res['ok'] ? "✅ " : "❌ ") . $res['msg'];
    }

    /* === Benutzer löschen === */ elseif ($action === 'delete_user') {
      if (!$canViewOthers)
        throw new Exception("Keine Berechtigung, Benutzer zu löschen.");
      ...
      if (!$ok)
        throw new Exception("Löschen fehlgeschlagen (evtl. referenzierte Daten).");

      foreach (['profilbild', 'firmenlogo', 'titelbild'] as $k)
        remove_local_upload($delRow[$k] ?? null, $PREFIX);

      $flash = "🗑️ Benutzer #$delId gelöscht.";
    }

    /* === Benutzer sperren / entsperren === */ elseif ($action === 'toggle_block') {
      if (!$canViewOthers)
      ...
      if ($bId <= 0)
        throw new Exception("Ungültige ID.");
      if ($bId === $sid)
        throw new Exception("Du kannst dich nicht selbst sperren.");
    ...
    /* === Benutzer anlegen === */ elseif ($action === 'create_user') {
      if (!$canViewOthers)
        throw new Exception("Keine Berechtigung, Benutzer anzulegen.");
      ...
      $pb_w = max(80, min(400, $pb_w));
      $lg_w = max(80, min(400, $lg_w));
      $tb_h = max(100, min(800, $tb_h));
      ...
      if (in_array('mieter_phase', $benutzerCols, true)) {
        $cols[] = 'mieter_phase';
        $types .= 's';
        $vals[] = trim($_POST['new_mieter_phase'] ?? 'interessent');
      }
      if (in_array('wohnung_id', $benutzerCols, true)) {
        $cols[] = 'wohnung_id';
        $types .= 'i';
        $vals[] = (int) ($_POST['new_wohnung_id'] ?? 0) ?: null;
      }
      if (in_array('kontaktweg', $benutzerCols, true)) {
        $cols[] = 'kontaktweg';
        $types .= 's';
        $vals[] = trim($_POST['new_kontaktweg'] ?? 'email');
      }

      if (in_array('person_type_id', $benutzerCols, true)) {
        $cols[] = 'person_type_id';
        $types .= 'i';
        $vals[] = $new_person_type_id ?: null;
      }
      if (in_array('person_status_id', $benutzerCols, true)) {
        $cols[] = 'person_status_id';
        $types .= 'i';
        $vals[] = $new_person_status_id ?: null;
      }

      if (in_array('projekt_id', $benutzerCols, true)) {
        $cols[] = 'projekt_id';
        $types .= 'i';
        $vals[] = (int) ($_POST['new_projekt_id'] ?? 0) ?: null;
      }
      if (in_array('business_type', $benutzerCols, true)) {
        $cols[] = 'business_type';
        $types .= 's';
        $vals[] = trim($_POST['new_business_type'] ?? 'standard');
      }

      if (in_array('objekt_id', $benutzerCols, true)) {
        $cols[] = 'objekt_id';
        $types .= 'i';
        $vals[] = (int) ($_POST['new_objekt_id'] ?? 0) ?: null;
      }

      $cols[] = 'profile_updated_at';
      $types .= 's';
      ...
      $bkp_id = (int) ($_POST['new_bkp_id'] ?? 0);
      $proj_id = (int) ($_POST['new_projekt_id'] ?? 0);
      if ($newId > 0 && $proj_id > 0) {
        $stUp = $mysqli->prepare("INSERT INTO unternehmer_projekte (benutzer_id, projekt_id, bkp_id) VALUES (?,?,?)");
        $stUp->bind_param("iii", $newId, $proj_id, $bkp_id);
        $stUp->execute();
        $stUp->close();
      }
      ...
      $target_pid = (int) ($_POST['new_projekt_id'] ?? 0);
    ...
    /* === Profil-Update === */ elseif ($action === 'save_profile') {
      $targetId = (int) ($_POST['id'] ?? 0);
      if ($targetId !== $sid && !$canViewOthers)
        throw new Exception("Keine Berechtigung, andere Profile zu ändern.");
      ...
      $stmt->close();
      if (!$targetUser)
        throw new Exception("Ziel-Benutzer nicht gefunden.");
      ...
      if ($firmaSelectId > 0) {
        ...
        if ($f) {
          if (($data['firma_name'] ?? '') === '')
            $data['firma_name'] = $f['name'] ?? '';
          if (($data['firma_adresse'] ?? '') === '')
            $data['firma_adresse'] = $f['adresse'] ?? '';
          if (($data['firma_telefon'] ?? '') === '')
            $data['firma_telefon'] = $f['telefon'] ?? '';
          if (($data['firma_email'] ?? '') === '')
            $data['firma_email'] = $f['email'] ?? '';
          if (($data['firma_website'] ?? '') === '')
            $data['firma_website'] = $f['website'] ?? '';
        }
      }
      ...
      if (!$firmenlogo && !empty($f) && !empty($f['logo']))
        $firmenlogo = sanitize_image_url($f['logo'], $PREFIX);
      $titelbild = $newT ?? $oldT;

      // Löschen nur, wenn KEIN neues hochgeladen wurde
      if ($delP && !$newP) {
        remove_local_upload($oldP, $PREFIX);
        $profilbild = null;
      }
      if ($delL && !$newL) {
        remove_local_upload($oldL, $PREFIX);
        $firmenlogo = null;
      }
      if ($delT && !$newT) {
        remove_local_upload($oldT, $PREFIX);
        $titelbild = null;
      }
      ...
      $types = str_repeat('s', 20); // 20 Strings
      $vals = [
        $data['name'],
        $data['email'],
        $finalRole,
      ...
      if (in_array('projekt_id', $benutzerCols, true)) {
        $sql .= ", projekt_id=?";
        $types .= 'i';
        $vals[] = (int) ($_POST['projekt_id'] ?? 0) ?: null;
      }
      if (in_array('objekt_id', $benutzerCols, true)) {
        $sql .= ", objekt_id=?";
        $types .= 'i';
        $vals[] = (int) ($_POST['objekt_id'] ?? 0) ?: null;
      }
      if (in_array('wohnung_id', $benutzerCols, true)) {
        $sql .= ", wohnung_id=?";
        $types .= 'i';
        $vals[] = (int) ($_POST['wohnung_id'] ?? 0) ?: null;
      }
...
<style>
  ...
  .accordion-section.active .accordion-header .icon {
  ...
  .accordion-section.active .user-count-pill {
    background: #2563eb;
    color: #fff;
  }
  ...
  <header class="hero">
    ...
    <div class="hero-right">
      ...
      <?php if (in_array($currentRole, ['admin', 'superadmin'], true)): ?>
        <button class="btn btn-teal btn-small"
          onclick="document.getElementById('create-user-section').scrollIntoView({behavior:'smooth'})">➕ Neu</button>
      <?php endif; ?>
    </div>
  </header>
      ...
      <form method="post" autocomplete="on" enctype="multipart/form-data" id="create-user-form">
          ...
          <div class="modern-grid">
            <div class="modern-field">
              ...
              <div class="modern-input-group">
                ...
                <?php $lvl = 'private'; ?>
                <button type="button" class="icon-toggle-new" data-for="new_name"
                  title="<?= h($LEVEL_LABEL[$lvl]) ?>"><?= $LEVEL_ICON[$lvl] ?></button>
              </div>
            ...
            <div class="modern-field">
              ...
              <div class="modern-input-group">
                ...
                <button type="button" class="icon-toggle-new" data-for="new_email"
                  title="<?= h($LEVEL_LABEL[$lvl]) ?>"><?= $LEVEL_ICON[$lvl] ?></button>
              </div>
            ...
            <div class="modern-field">
              ...
              <div class="modern-input-group">
                <select id="new_business_type" name="new_business_type" onchange="handleUserTypeChange(this.value)"
                  required>
        ...
        <div class="modern-form-section">
          ...
          <div class="modern-grid">
            <div class="modern-field" id="wrap_mieter_phase" style="display:none;">
              ...
              <div class="modern-input-group">
                <select id="new_mieter_phase" name="new_mieter_phase">
                  ...
                  <option value="bewerber" <?= $prePhase === 'bewerber' ? 'selected' : '' ?>>📝 Bewerber</option>
            ...
            <?php if (isset($_GET['wohnung_id'])): ?>
              <input type="hidden" name="new_wohnung_id" value="<?= (int) $_GET['wohnung_id'] ?>">
            <?php endif; ?>
            ...
            <?php if (in_array('wohnung_id', $benutzerCols, true)): ?>
                ...
                <div class="modern-input-group">
                  <select id="new_projekt_id" name="new_projekt_id">
                    ...
                    <?php foreach ($projects as $p): ?>
                      <option value="<?= (int) $p['id'] ?>" <?= ($p['id'] == $preP ? 'selected' : '') ?>><?= h($p['name']) ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                </div>
              ...
              <div class="modern-field">
                ...
                <div class="modern-input-group">
                  <select id="new_objekt_id" name="new_objekt_id">
                    <option value="">— auswählen —</option>
                    <?php
                    if ($preP > 0 && isset($hierarchyJS['objects'][$preP])) {
                      foreach ($hierarchyJS['objects'][$preP] as $o) {
                        $sel = ($o['id'] == $preO) ? 'selected' : '';
                        echo '<option value="' . (int) $o['id'] . '" ' . $sel . '>' . h($o['name']) . '</option>';
                      }
                    } else {
                      echo '<option value="">— Erst Projekt wählen —</option>';
                    }
                    ?>
                  </select>
                </div>
              ...
              <div class="modern-field">
                ...
                <div class="modern-input-group">
                  <select id="new_wohnung_id" name="new_wohnung_id">
              ...
              <div class="modern-field">
                <label>Zuständig für Gewerk (BKP)</label>
                <div class="modern-input-group">
                  <select id="new_bkp_id" name="new_bkp_id">
                    <option value="">— Alle / Keines —</option>
                    <?php foreach ($bkp_codes_all as $b): ?>
                      <option value="<?= (int) $b['id'] ?>"><?= h($b['code'] . ' ' . $b['bezeichnung']) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
            ...
            <?php if (!empty($types)): ?>
              <div id="wrap_person_type" style="display:contents;">
                <div class="modern-field">
            ...
            <div class="modern-field" id="wrap_bkp_id" style="display:none;">
              <label>BKP 2 – Gebäude (Zuweisung)</label>
              <div class="modern-input-group">
            ...
            foreach ($createFields as $cf) {
              ...
              <div class="modern-field">
                ...
                <div class="modern-input-group">
                  ...
                  <button type="button" class="icon-toggle-new" data-for="<?= h($id) ?>"
                    title="<?= h($LEVEL_LABEL[$lvl]) ?>"><?= $LEVEL_ICON[$lvl] ?></button>
                </div>
        ...
        <div class="modern-form-section">
          ...
          <div
            style="margin-top:16px; padding:12px; border-left:4px solid #3b82f6; background:#eff6ff; border-radius:4px;">
            <label style="display:flex;align-items:center;gap:8px; cursor:pointer; margin:0;">
              <input type="checkbox" id="save_firma_template" name="save_firma_template" value="1">
              <span style="font-size:13px; font-weight:600; color:#1e40af;">Firma als Vorlage in Stammdaten speichern (nur
                wenn oben keine bestehende Firma ausgewählt ist)</span>
            </label>
          </div>
        </div>
        ...
        <div class="modern-form-section">
          ...
          <div class="modern-grid">
            ...
            <div class="modern-field">
              ...
              <div class="modern-input-group">
                <div style="flex:1;">
                  <input type="file" name="new_profilbild" accept="image/*"
                    style="width:100%; border:1px solid #e2e8f0; border-radius:8px; padding:6px; margin-bottom:8px;">
                  <div style="display:flex; align-items:center; gap:8px; font-size:12px; color:#64748b;">
                    Größe: <input type="number" id="new_profilbild_max_size_num" name="new_profilbild_max_size_num"
                      min="80" max="400" step="5" value="160"
                      style="width:60px; padding:2px 4px; border:1px solid #cbd5e1; border-radius:4px;"> px
                  </div>
                </div>
                <?php $lvl = 'private'; ?>
                <button type="button" class="icon-toggle-new" data-for="new_profilbild"
                  title="<?= h($LEVEL_LABEL[$lvl]) ?>"><?= $LEVEL_ICON[$lvl] ?></button>
            ...
            <div class="modern-field">
              ...
              <div class="modern-input-group">
                <div style="flex:1;">
                  <input type="file" name="new_firmenlogo" accept="image/*"
                    style="width:100%; border:1px solid #e2e8f0; border-radius:8px; padding:6px; margin-bottom:8px;">
                  <div style="display:flex; align-items:center; gap:8px; font-size:12px; color:#64748b;">
                    Größe: <input type="number" id="new_firmenlogo_max_size_num" name="new_firmenlogo_max_size_num"
                      min="80" max="400" step="5" value="160"
                      style="width:60px; padding:2px 4px; border:1px solid #cbd5e1; border-radius:4px;"> px
                  </div>
                </div>
                <button type="button" class="icon-toggle-new" data-for="new_firmenlogo"
                  title="<?= h($LEVEL_LABEL[$lvl]) ?>"><?= $LEVEL_ICON[$lvl] ?></button>
            ...
            <div class="modern-field">
              ...
              <div class="modern-input-group">
                <div style="flex:1;">
                  <input type="file" name="new_titelbild" accept="image/*"
                    style="width:100%; border:1px solid #e2e8f0; border-radius:8px; padding:6px; margin-bottom:8px;">
                  <div style="display:flex; align-items:center; gap:8px; font-size:12px; color:#64748b;">
                    Höhe: <input type="number" id="new_titelbild_max_height_num" name="new_titelbild_max_height_num"
                      min="100" max="800" step="10" value="260"
                      style="width:60px; padding:2px 4px; border:1px solid #cbd5e1; border-radius:4px;"> px
                  </div>
                </div>
                <button type="button" class="icon-toggle-new" data-for="new_titelbild"
                  title="<?= h($LEVEL_LABEL[$lvl]) ?>"><?= $LEVEL_ICON[$lvl] ?></button>
                <input type="hidden" name="new_vis_titelbild" id="new_vis_titelbild" value="private">
              </div>
            </div>
          </div>
          <div style="margin-top:12px; font-size:11px; color:#94a3b8; text-transform:uppercase; letter-spacing:0.05em;">
            Medien werden beim Upload automatisch auf ≤ 1 MB komprimiert.</div>
        </div>

        <div
          style="padding:20px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;">
          <label style="display:inline-flex;align-items:center;gap:8px; cursor:pointer; margin:0;">
            <input type="checkbox" id="send_invite_now" name="send_invite_now" value="1" checked
              style="width:18px;height:18px;">
            <span style="font-size:14px; font-weight:700; color:#0f172a;">📧 Nach dem Anlegen automatisch E-Mail mit
              Einladungslink senden</span>
          </label>
          <button class="btn-premium" type="submit"
            style="padding:14px 40px; font-size:16px; border:none; cursor:pointer;">Benutzer Anlegen &rarr;</button>
        </div>
    ...
    <div class="card">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
        ...
        <div style="display:flex; gap:10px;">
          <input type="text" id="userListSearch" placeholder="Suchen..."
            style="padding:8px 12px; border-radius:8px; border:1px solid #e2e8f0; font-size:13px;">
        </div>
      </div>
        ...
        <div class="accordion-section" id="group-<?= h($key) ?>">
          ...
          <div class="accordion-content">
            <?php if (empty($g['users'])): ?>
              <div style="padding:20px; text-align:center; color:#94a3b8; font-style:italic; font-size:13px;">Keine Benutzer
                in dieser Kategorie.</div>
            <?php else: ?>
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <tr>
                      ...
                      <td>
                        <div style="display:flex; align-items:center; gap:8px;">
                          <?php if ($pu): ?><img src="<?= h($pu) ?>"
                              style="width:24px; height:24px; border-radius:50%; object-fit:cover;"><?php endif; ?>
                      ...
                      <td>
                        <?php if ((int) ($u['is_blocked'] ?? 0) === 1): ?>
                          <span style="color:#ef4444; font-size:10px; font-weight:800; text-transform:uppercase;">🚫
                            Gesperrt</span>
                        <?php else: ?>
                        ...
                        <div style="font-size:10px; color:#64748b; margin-top:2px;">
                          ...
                          <?php if (!empty($u['bkp_code_current'])): ?>
                            <span
                              style="background:#f1f5f9; color:#475569; padding:2px 6px; border-radius:4px; font-weight:800; margin-left:5px;">BKP
                              <?= h($u['bkp_code_current']) ?></span>
                          <?php endif; ?>
                        </div>
                      </td>
                      <td>
                        <div style="display:flex; gap:6px;">
                          ...
                          <form method="post" style="display:inline;">
                            ...
                            <input type="hidden" name="id" value="<?= (int) $u['id'] ?>">
                            <button class="btn-outline btn-small" type="submit"
                              title="<?= (int) $u['is_blocked'] ? 'Entsperren' : 'Sperren' ?>">
                              <?= (int) $u['is_blocked'] ? '🔓' : '🚫' ?>
                            </button>
                          </form>
                        </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupHierarchy(prefix) {
      ...
      async function updateObjects(pId, selectedId) {
        pId = String(pId || '');
        oSel.innerHTML = '<option value="">— laden... —</option>';
        uSel.innerHTML = '<option value="">— zuerst Haus wählen —</option>';
        oSel.disabled = true;
        uSel.disabled = true;

        if (!pId || pId === '0') {
          oSel.innerHTML = '<option value="">— Erst Projekt wählen —</option>';
          return;
        }

        try {
          const resp = await fetch('benutzer.php?ajax_hierarchy=1&pId=' + pId);
          const data = await resp.json();
          oSel.innerHTML = '<option value="">— auswählen —</option>';
          if (data.objects && data.objects.length > 0) {
            ...
            if (data.objects.length === 1 && !selectedId) oSel.selectedIndex = 1;
            updateUnits(oSel.value);
          } else {
            oSel.innerHTML = '<option value="">— Keine Häuser gefunden —</option>';
          }
        } catch (e) { console.error("MySQL Sync Error", e); }
      }

      async function updateUnits(oId, selectedId) {
        uSel.innerHTML = '<option value="">— laden... —</option>';
        if (!oId) {
          uSel.disabled = true;
          uSel.innerHTML = '<option value="">— zuerst Haus wählen —</option>';
          return;
        ...
        try {
          const resp = await fetch('benutzer.php?ajax_hierarchy=1&oId=' + oId);
          const data = await resp.json();
          uSel.innerHTML = '<option value="">— auswählen —</option>';
          if (data.units && data.units.length > 0) {
            ...
            uSel.disabled = true;
            uSel.innerHTML = '<option value="">— Keine Einheiten gefunden —</option>';
          }
        } catch (e) { console.error("MySQL Sync Error", e); }
      }

      pSel.addEventListener('change', () => {
        console.log("Hierarchy: Project changed to", pSel.value);
        updateObjects(pSel.value);
      });
      oSel.addEventListener('change', () => {
        console.log("Hierarchy: Object changed to", oSel.value);
        updateUnits(oSel.value);
      });

      // Initial Trigger nach kurzem Delay um DOM-Ready zu garantieren
      setTimeout(() => {
        const curPid = pSel.value;
        const curOid = oSel.value;
        if (curPid && !curOid) {
          console.log("Hierarchy: Force initial trigger for", curPid);
          updateObjects(curPid, (document.getElementById(prefix + 'objekt_id_val') || {}).value);
        } else if (curPid && curOid) {
          oSel.disabled = false;
          updateUnits(curOid, (document.getElementById(prefix + 'wohnung_id_val') || {}).value);
        }
      }, 300);
    }
  ...
  function handleUserTypeChange(val) {
    ...
    const wrapType = document.getElementById('wrap_person_type');
    ...
    if (wrapType) wrapType.style.display = 'none';

    if (val === 'standard') {
      // Team / Mitarbeiter
      roleSel.value = 'benutzer';
      if (wrapType) wrapType.style.display = 'contents';
    }
