...
<style>
    ...
    .pendenzen-wrap { max-width: 96%; margin: 0 auto; }
    ...
    .pendenzen-dashboard-field select option { color: var(--text-main); background: #fff; }
    .pendenzen-quick-btn {
        background: #fff;
        color: #0f766e;
        font-weight: 700;
    ...
    .pendenzen-quick-btn:hover { background: #f1f5f9; transform: translateY(-1px); }
    ...
    .pendenzen-sub { margin-top: 4px; color: var(--text-muted); font-size: 14px; }

    .pendenzen-content { padding: 28px; }
    .pendenzen-msg {
        padding: 16px;
        border-radius: 10px;
        margin-bottom: 24px;
    ...
    .pendenzen-msg.ok { background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; }
    .pendenzen-msg.err { background: #fef2f2; color: #991b1b; border: 1px solid #fecaca; }

    /* Form Styles */
    .pendenzen-form { display: flex; flex-direction: column; gap: 28px; }
    .pendenzen-grid {
        display: grid;
    ...
    .pendenzen-field textarea { min-height: 120px; resize: vertical; }
    ...
    /* List & Table Styling */
    .pendenzen-list { margin-top: 40px; }
    ...
    .pendenzen-table-tools > * { flex: 1; min-width: 150px; }
    ...
    .pendenzen-table { width: 100%; border-collapse: collapse; min-width: 1000px; }
    .pendenzen-table th {
        background: #f1f5f9;
    ...
    .pendenzen-table th:hover { background: #e2e8f0; color: var(--text-main); }
    .pendenzen-table td {
        padding: 16px;
        border-bottom: 1px solid var(--border-color);
        font-size: 14px;
    ...
    .pendenzen-table tr:last-child td { border-bottom: 0; }
    .pendenzen-table tr:hover td { background: #f8fafc; }
    ...
    .pendenzen-pill.offen { background: #dcfce7; color: #166534; }
    .pendenzen-pill.bearbeitung { background: #fef9c3; color: #854d0e; }
    .pendenzen-pill.erledigt { background: #dbeafe; color: #1e40af; }
    .pendenzen-pill.archiviert { background: #f1f5f9; color: #475569; }
    .pendenzen-small { font-size: 12px; color: var(--text-muted); margin-top: 4px; line-height: 1.4; }

    /* Mobile Optimization */
    @media (max-width: 1024px) {
        .pendenzen-grid { grid-template-columns: 1fr 1fr; }
    }

    @media (max-width: 900px) {
        .pendenzen-page { padding: 12px; }
        .pendenzen-dashboard { padding: 20px 16px; }
        .pendenzen-dashboard-top { flex-direction: column; align-items: stretch; }
        .pendenzen-dashboard-actions { justify-content: flex-start; }
        .pendenzen-chip-link { flex: 1; justify-content: center; min-width: 140px; }
        .pendenzen-dashboard-grid { grid-template-columns: 1fr; }
        
        .pendenzen-grid, .pendenzen-grid-2 { grid-template-columns: 1fr; }
        .pendenzen-content { padding: 20px 16px; }
        .pendenzen-head { padding: 20px 16px; }
        
        .pendenzen-table-tools > * { flex: none; width: 100%; }
        .pendenzen-btn { width: 100%; padding: 16px; }
        ...
        .pendenzen-table-wrap { border: 0; background: transparent; box-shadow: none; overflow: visible; }
        .pendenzen-table, 
        ...
        .pendenzen-table td:last-child { border-bottom: 0; }
        ...
        .pendenzen-table td strong { font-size: 14px; color: var(--text-main); }
        .pendenzen-pill { padding: 2px 8px; font-size: 11px; }
        .pendenzen-small { margin-top: 2px; font-size: 12px; }
    }
    .form-section {
        background: #fff;
        border: 1px solid var(--border-color);
    ...
    .collapsed #formContent { display: none; }
    .hidden { display: none; }
    #toggleForm:hover { background: #f8fafc; }
...
<div class="pendenzen-page">
    <div class="pendenzen-wrap">
        ...
        <div class="pendenzen-card">
            <div class="pendenzen-head" style="cursor:pointer; display:flex; justify-content:space-between; align-items:center;" id="toggleForm">
                <div>
            ...
            <div class="pendenzen-content" id="formContent">
                <?php if ($success !== ''): ?>
                        ...
                        <table id="pendenzenTable" class="pendenzen-table">
                            <thead>
                                <tr>
                                    ...
                                    <th data-sort="text" data-label="Projekt">Projekt</th>
                                    <th data-sort="text" data-label="Wohnung">Wohnung</th>
                                    <th data-sort="text" data-label="Status">Status</th>
                                    <th data-sort="number" data-label="Prio">Wichtigkeit</th>
                                    <th data-sort="text" data-label="Zuständig">Zuständig</th>
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <?php foreach ($recent as $row): 
                                        ...
                                        if (str_contains($statusLow, 'offen')) $statusClass = 'offen';
                                        elseif (str_contains($statusLow, 'bearbeitung')) $statusClass = 'bearbeitung';
                                        elseif (str_contains($statusLow, 'erledigt')) $statusClass = 'erledigt';
                                        elseif (str_contains($statusLow, 'archiv')) $statusClass = 'archiviert';
                                    ?>
                                        <tr
                                        ...
                                        >
                                            ...
                                            <td data-label="Projekt"><?php echo h($row['projekt_name']); ?></td>
                                            <td data-label="Wohnung"><?php echo h($row['wohnung_name']); ?></td>
                                            <td data-label="Status"><span class="pendenzen-pill <?php echo $statusClass; ?>"><?php echo h($row['status']); ?></span></td>
                                            <td data-label="Prio">
                                                <span class="pendenzen-pill" style="background:<?php echo ((int)$row['wichtigkeit'] >= 4) ? '#fee2e2' : '#f1f5f9'; ?>; color:<?php echo ((int)$row['wichtigkeit'] >= 4) ? '#991b1b' : '#475569'; ?>;">
                                                    Prio <?php echo h($row['wichtigkeit']); ?>
                                                </span>
                                            </td>
                                            <td data-label="Zuständig"><?php echo h($row['zustaendig_name']); ?></td>
                                            <td data-label="Start"><div class="pendenzen-small"><?php echo h($row['startdatum']); ?></div></td>
                                            <td data-label="Ende"><div class="pendenzen-small"><?php echo h($row['enddatum'] ?: '—'); ?></div></td>
                                            <td data-label="Zeit"><div class="pendenzen-small"><?php echo h($row['uhrzeit']); ?></div></td>
                                            <td data-label="Dauer"><div class="pendenzen-small"><?php echo h($row['dauer']); ?></div></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </tbody>
