        ...
        <tr>
          <?php foreach ($cols as $c):
            ...
            if ($c === 'status') {
               ...
               $imgRes = $mysqli->query("SELECT pfad, typ, mimetype FROM pendenz_dateien WHERE pendenz_id=".(int)$r['id']." AND ($typeFilter) ORDER BY CASE WHEN titel='Plan-Ausschnitt' THEN 1 ELSE 0 END ASC, is_cover DESC, id ASC LIMIT $limit");
              ...
              <?php foreach ($cols as $c):
                ...
                if (str_starts_with($c, 'json:')) {
                  ...
                  $disp = h((string)($jx[$fld] ?? '—'));
                    ...
                    $resM = $mysqli->query("SELECT pfad, mimetype FROM pendenz_dateien WHERE pendenz_id=".(int)$r['id']." ".($c==='pdf'?" AND mimetype LIKE '%pdf%'":"")." ORDER BY CASE WHEN titel='Plan-Ausschnitt' THEN 1 ELSE 0 END ASC, is_cover DESC, id ASC LIMIT 1");
