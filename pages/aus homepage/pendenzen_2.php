    ...
    (function () {
        ...
        if (table) {
            ...
            table.querySelectorAll('th').forEach((th, i) => {
                const colKey = 'p_col_w_' + i + '_' + (th.dataset.key || th.textContent.trim().substring(0, 10));

                // Restore saved width
                const savedW = localStorage.getItem(colKey);
                if (savedW) { th.style.width = savedW; }

                const r = document.createElement('div');
                r.style.cssText = 'position:absolute;right:0;top:0;width:5px;height:100%;cursor:col-resize;user-select:none;z-index:1';
                th.appendChild(r);
                let x = 0, w = 0;
                const md = (e) => { x = e.clientX; w = th.offsetWidth; document.addEventListener('mousemove', mm); document.addEventListener('mouseup', mu); e.preventDefault(); };
                const mm = (e) => { const nw = w + (e.clientX - x); th.style.width = nw + 'px'; };
                const mu = () => {
                    document.removeEventListener('mousemove', mm);
                    document.removeEventListener('mouseup', mu);
                    localStorage.setItem(colKey, th.style.width);
                };
        ...
        if (lb && lbImg) {
            document.querySelectorAll('.pendenzen-table-img').forEach(img => {
        ...
        document.querySelectorAll('.btn-edit-professional').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (!e.ctrlKey && !e.shiftKey && !e.metaKey) {
                    e.preventDefault();
                    pendenzNeuFrame.src = 'pendenz_neu.php?embed=1&id=' + id;
                    if (formContent && formContent.style.display === 'none') toggleForm?.click();
                    setTimeout(() => { (document.getElementById('formTopMarker') || toggleForm)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
                }
            });
        });
