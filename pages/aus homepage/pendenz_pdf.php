...
/**
 * Hilfsfunktion um Benutzer inkl. seiner primären Firma zu laden
 */
function get_user_with_company($mysqli, $id) {
    if (!$id) return null;
    $sql = "SELECT b.*, 
    ...
    if ($u) {
        // Aggressive Mappings: Firmen-Daten überschreiben Benutzer-Daten falls verknüpft
        if (!empty($u['f_name'])) {
            $u['firma_name'] = $u['f_name'];
        }
        if (!empty($u['f_email'])) {
            $u['firma_email'] = $u['f_email'];
        }
        if (!empty($u['f_telefon'])) {
            $u['firma_telefon'] = $u['f_telefon'];
        }
        if (!empty($u['f_ort'])) {
            $u['firma_ort'] = $u['f_ort']; 
            $u['firma_plz'] = ''; 
        ...
        if (!empty($u['f_website'])) $u['firma_website'] = $u['f_website'];
    }
    return $u;
}
...
if ((int)($p['public_enabled']??0)===1 && !empty($p['public_token'])) {
  $publicUrl = $host . rtrim(site_prefix(),'/') . "/pages/pendenz_public.php?t=" . rawurlencode($p['public_token']);
}
// Immer einen QR-Code erzeugen (wenn kein public link, dann permalink)
$qrData = $publicUrl ?: $host . rtrim(site_prefix(),'/') . '/pages/pendenz_show.php?id=' . $id;
...
<?php if ($maskCfg && isset($maskCfg['blocks'])): ?>
  <div class="absolute-container" style="padding: <?= $maskCfg['margin'] ?? 0 ?>px;">
    <?php foreach ($maskCfg['blocks'] as $block): 
        ...
        $coverImg = $hero ? $makeImgTag($hero['pfad'], 'width:100%; max-height:220px; object-fit:cover;') : '';
        $erstesImg = !empty($imgs[0]) ? $makeImgTag($imgs[0]['pfad'], 'max-width:100%; max-height:150px;') : '';
        ...
        $reps = [
          '{TITEL}' => '<h1>'.htmlspecialchars($p['titel']).'</h1>',
          '{TITLE}' => '<h1>'.htmlspecialchars($p['titel']).'</h1>',
          '{PROJEKT}' => htmlspecialchars($p['projekt_name']??''),
          '{PROJECT}' => htmlspecialchars($p['projekt_name']??''),
          '{OBJEKT}' => htmlspecialchars($p['objekt_name']??''),
          '{WOHNUNG}' => htmlspecialchars($p['wohnung_name']??''),
          '{RAUM}' => htmlspecialchars($p['raum_name']??''),
          '{DAUER}' => htmlspecialchars($p['dauer']??''),
          '{STATUS}' => $status,
          '{SICHTBARKEIT}' => htmlspecialchars($p['sichtbarkeit']??''),
          '{VORGANGSART}' => htmlspecialchars($p['vorgangsart_name']??''),
          '{UHRZEIT}' => !empty($p['uhrzeit']) ? htmlspecialchars(substr($p['uhrzeit'],0,5)) : '',
          '{TAGESZEIT}' => htmlspecialchars($p['tageszeit']??''),
          '{VORGAENGER_ID}' => htmlspecialchars($p['vorgaenger_id']??''),
          '{KURZBESCHREIBUNG}' => '<div><strong>Kurzbeschreibung</strong><br>'.nl2br(htmlspecialchars($p['kurzbeschreibung']??'')).'</div>',
          '{DESCRIPTION}' => '<div><strong>Beschreibung</strong><br>'.nl2br(htmlspecialchars($p['langbeschreibung']??'')).'</div>',
          '{LANGBESCHREIBUNG}' => '<div><strong>Beschreibung</strong><br>'.nl2br(htmlspecialchars($p['langbeschreibung']??'')).'</div>',
          '{NOTIZ}' => nl2br(htmlspecialchars($p['notiz']??'')),
          '{NOTIZEN}' => nl2br(htmlspecialchars($p['notiz']??'')),
          '{DATUM}' => date('d.m.Y'),
          ...
          '{VERANTWORTLICH}' => $zName,
          '{VERANTWORTLICHER}' => $zName,
          '{ERSTELLT_VON}' => htmlspecialchars($createdUser['name']??''),
          '{COVER}' => $coverImg,
          ...
          '{ZUSTAENDIG_PLZ}' => htmlspecialchars($z['firma_plz']??''),
          '{ZUSTAENDIG_ORT}' => htmlspecialchars($z['firma_ort']??''),
          '{ZUSTÄNDIG_PLZ}' => htmlspecialchars($z['firma_plz']??''),
          '{ZUSTÄNDIG_ORT}' => htmlspecialchars($z['firma_ort']??''),
          '{ZUSTAENDIG_FIRMA_ORT}' => htmlspecialchars($z['firma_ort']??''),
          '{ZUSTÄNDIG_FIRMA_ORT}' => htmlspecialchars($z['firma_ort']??''),
        ];
        ...
        if (strpos($text, '{BILDER}') !== false) {
            ...
            foreach ($imgs as $i) {
                ...
                if ($data) {
                    ...
                    $grid .= '<div class="cell"><img src="' . $b64 . '" style="max-width:100%; max-height:110px; object-fit:cover;"></div>';
                    $count++;
        ...
        // 2. Zeilenumbrüche beachten (Enter = Neue Zeile)
        $text = nl2br($text);
    ?>
      <?php 
...
<?php else: ?>
  ...
  <div class="row clearfix">
    ...
    <div class="right">
      <?php if($qrUrl): ?>
        <div style="text-align:right;"><img src="<?= htmlspecialchars($qrUrl) ?>" style="width:70px;height:70px;border:1px solid #eee;border-radius:6px;"></div>
      <?php endif; ?>
    </div>
  </div>

  <?php if($hero && is_file(__DIR__.'/../'.ltrim($hero['pfad'],'/'))):
    $abs=realpath(__DIR__.'/../'.ltrim($hero['pfad'],'/')); $data=@file_get_contents($abs);
    if($data): $b64='data:'.($hero['mimetype']?:'image/jpeg').';base64,'.base64_encode($data); ?>
      <img class="hero" src="<?= $b64 ?>">
  <?php endif; endif; ?>

  <?php if (in_array('meta',$view['sections'],true) && $metaItems): ?>
    <h2>Meta</h2>
    <div class="meta">
      <?php foreach($metaItems as $it): ?>
        <div class="item"><div class="k"><?= htmlspecialchars($it['label']) ?></div><div class="v"><strong><?= htmlspecialchars($it['value']) ?></strong></div></div>
      <?php endforeach; ?>
    </div>
  <?php endif; ?>
