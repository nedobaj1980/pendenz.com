...
if (is_file($__utPath)) {
    ...
    if (function_exists('app_log')) app_log('Hinweis: includes/user_taxonomy.php fehlt â€“ Typ/Status-UI deaktiviert.');
}
...
function firmen_insert_dynamic(mysqli $db, array $data): ?int {
    ...
    foreach (['adresse','ort','telefon','email','website','logo'] as $c) {
...
$FIELDS = [
  ...
  'adresse'         => 'Adresse',
  'ort'             => 'Ort',
  'firma_name'      => 'Firma â€“ Name',
  'firma_adresse'   => 'Firma â€“ Adresse',
  'firma_ort'       => 'Firma â€“ Ort',
  'firma_telefon'   => 'Firma â€“ Telefon',
  'firma_email'     => 'Firma â€“ E-Mail',
  'firma_website'   => 'Firma â€“ Website',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    elseif ($action === 'create_user') {
      ...
      $firma_ort     = $mf['ort']     !== '' ? $mf['ort']     : ($f['ort']     ?? null);
      ...
      $types = 'sssssssssssssssss'; // 17 Strings
      $vals  = [
        $new_name,$new_email,$new_role,
        $new_telefonnummer,$new_adresse,$new_ort,
        $firma_name,$firma_adresse,$firma_ort,$firma_telefon,$firma_email,$firma_website,
    ...
    elseif ($action === 'save_profile') {
      ...
      if ($firmaSelectId > 0) {
        ...
        if ($f) {
          ...
          if (($data['firma_ort']      ?? '') === '') $data['firma_ort']     = $f['ort']     ?? '';
...
<div class="container" id="profile-root" data-prefix="<?= h($PREFIX) ?>">
    ...
    <form method="post" autocomplete="on" enctype="multipart/form-data" id="create-user-form">
      ...
      <div class="modern-form-section">
        ...
        <div class="modern-grid">
          ...
          <div class="modern-field">
            <label>E-Mail (oder Telefonnummer unten angeben)</label>
            <div class="modern-input-group">
        ...
        <div class="modern-grid">
          ...
          <?php if (!empty($types)): ?>
            ...
            <div class="modern-field" style="grid-column:1 / -1;">
              <label>Weitere Personen/Sub-Typen</label>
              <div id="newPersonTypeMembershipRows" style="display:flex; flex-direction:column; gap:10px;"></div>
              <div style="margin-top:10px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                <button type="button" id="addNewPersonTypeMembership" class="btn btn-sm">+ Weiteren Personen/Sub-Typ hinzufügen</button>
      ...
      <div class="modern-form-section">
        <div class="modern-form-title">☎️ Weitere Kontaktdaten & Info</div>
        <div class="modern-grid">
          <?php
            $createFields = [
  ...
  <div class="card">
    ...
    $groups = [
        'mitarbeiter' => ['label' => 'Mitarbeiter / Verwaltung', 'users' => [], 'icon' => '🏢'],
        'unternehmer' => ['label' => 'Unternehmer / Handwerker', 'users' => [], 'icon' => '🏗️'],
        'mieter'      => ['label' => 'Mieter & Bewohner', 'users' => [], 'icon' => '🏠'],
        'bewerber'    => ['label' => 'Interessenten & Bewerber', 'users' => [], 'icon' => '📋'],
        'partner'     => ['label' => 'Partner & Externe', 'users' => [], 'icon' => '🤝'],
    ...
    foreach($allUsers as $u) {
        ...
        $tName = mb_strtolower($u['primary_type_name'] ?? '');
        $phase = $u['mieter_phase'] ?? '';
        $bType = $u['business_type'] ?? 'standard';
        $role  = $u['rolle'] ?? 'benutzer';

        if ($tName === 'unternehmer' || $bType === 'handwerker') {
            $groups['unternehmer']['users'][] = $u;
        } elseif ($tName === 'mieter' || $phase === 'mieter' || $phase === 'vormieter' || $phase === 'gekündigt') {
            $groups['mieter']['users'][] = $u;
        } elseif ($tName === 'interessent' || $phase === 'interessent' || $phase === 'bewerber' || $bType === 'mietinteressent') {
            $groups['bewerber']['users'][] = $u;
        } elseif ($tName === 'mitarbeiter' || $tName === 'verwaltung' || $bType === 'standard' || $role === 'admin' || $role === 'superadmin') {
            $groups['mitarbeiter']['users'][] = $u;
        } elseif ($tName === 'kunde' || $tName === 'lieferant' || $tName === 'eigentümer') {
            $groups['partner']['users'][] = $u;
        } else {
...
const statNew = document.getElementById('new_person_status_id');
  if (typeNew && statNew) {
    typeNew.addEventListener('change', ()=>{
      ...
      // Automatisierung: Sichtbarkeit & Rollen
      ...
      if (tnameLow.includes('mieter') || tnameLow.includes('interessent')) {
          if (roleSel) roleSel.value = 'gast';
          if (wrapPhase) wrapPhase.style.display = 'block';
      } else if (tnameLow.includes('handwerker') || tnameLow.includes('unternehmer') || tnameLow.includes('partner')) {
          if (roleSel) roleSel.value = 'gast';
          if (wrapBkp) wrapBkp.style.display = 'block';
      } else {
          if (roleSel) roleSel.value = 'benutzer';
      }
    });
  }
</script>
