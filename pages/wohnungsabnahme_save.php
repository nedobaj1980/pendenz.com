...
// AJAX upload photo from URL (bypasses browser CORS for drag and drop)
if ($action === 'upload_photo_url') {
    $url = $_POST['url'] ?? $_GET['url'] ?? '';
    if (empty($url)) {
        echo json_encode(['success' => false, 'error' => 'Keine URL übergeben']);
        exit;
    }
    ...
    try {
        ...
        if (strpos($url, 'data:') === 0) {
            ...
            $commaIdx = strpos($url, ',');
            if ($commaIdx !== false) {
                $meta = substr($url, 0, $commaIdx);
                $base64Data = substr($url, $commaIdx + 1);
                $imgData = base64_decode($base64Data);
                
                if (strpos($meta, 'image/png') !== false) $ext = 'png';
                elseif (strpos($meta, 'image/gif') !== false) $ext = 'gif';
                elseif (strpos($meta, 'image/webp') !== false) $ext = 'webp';
            } else {
                throw new Exception('Ungültige Data-URL.');
            }
        } else {
            // Standard HTTP/HTTPS URL - Bypass SSL verification to support XAMPP local environments on Windows
            ...
            if ($imgData === false) {
                throw new Exception('Bild konnte nicht von der URL geladen werden. Der Server oder SSL-Handshake blockiert die Anfrage.');
            }
        ...
        if ($imgData === false) {
            throw new Exception('Bilddaten konnten nicht ermittelt werden.');
        }
