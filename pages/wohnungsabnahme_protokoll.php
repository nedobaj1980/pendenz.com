                                        ...
                                        <div class="inline-defect-panel" id="defect_panel_<?= $num ?>" style="display: <?= $isDefect ? 'block' : 'none' ?>;">
                                            ...
                                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 15px;">
                                                ...
                                                <div class="meta-box">
                                                    ...
                                                    <div class="photo-upload-wrapper" data-num="<?= $num ?>" style="margin-top: 5px; display: flex; align-items: center; gap: 10px;">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Initialize drag & drop for photo uploads
        initPhotoDragDrop();
    });
    ...
    // Helper to upload a File object (works for both file input and drag/drop)
    function uploadPhotoFile(num, file, uploadBtn) {
        if (!file) return;
    ...
    function uploadRowPhoto(num, fileInput) {
        ...
        uploadPhotoFile(num, file, uploadBtn);
    }

    // Initialize drag & drop for photo uploads
    function initPhotoDragDrop() {
        const wrappers = document.querySelectorAll('.photo-upload-wrapper');
        wrappers.forEach(wrapper => {
            const num = wrapper.getAttribute('data-num');
            const uploadBtn = wrapper.querySelector('.btn-photo-upload');
            if (!uploadBtn) return;
            ...
            // Toggle highlight class on dragover
            ...
            wrapper.addEventListener('drop', async (e) => {
                ...
                // Way 1: User drops a real file (e.g. from desktop)
                if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                    uploadPhotoFile(num, e.dataTransfer.files[0], uploadBtn);
                } 
                // Way 2: User drags an image from another browser tab (Google Photos, search, etc.)
                else {
                    let src = '';
                    ...
                    if (htmlData) {
                        try {
                            ...
                            console.error("DOMParser error parsing drag HTML:", err);
                        }
                    ...
                    if (!src) {
                        ...
                        if (plainText && (plainText.startsWith('http://') || plainText.startsWith('https://') || plainText.startsWith('data:'))) {
                            src = plainText;
                        }
                    }
                    
                    // Decode HTML entities (e.g. &amp; to &) to prevent malformed Google Photos URLs
                    if (src && !src.startsWith('data:')) {
                        try {
                            const doc = new DOMParser().parseFromString(src, 'text/html');
                            src = doc.documentElement.textContent;
                        } catch (err) {
                            console.error("Error decoding HTML entities in image URL:", err);
                        }
                    }
                    
                    if (src) {
                        // Send the URL directly to the backend to download (bypasses CORS completely)
                        const originalText = uploadBtn.innerHTML;
                        uploadBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Lade Link...`;
                        
                        const formData = new FormData();
                        formData.append("url", src);
                        
                        fetch("wohnungsabnahme_save.php?action=upload_photo_url", {
                            method: "POST",
                            body: formData
                        })
                        .then(res => res.json())
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                showToast("Foto erfolgreich von Google Photos geladen!");
                            } else {
                                alert("Fehler beim Laden von Google Photos: " + data.error);
                            }
                        })
                        .catch(err => {
                            uploadBtn.innerHTML = originalText;
                            alert("Verbindungsfehler beim Laden des Links.");
                        });
                    } else {
                        alert("Es konnte kein Bildlink aus diesem Drag-Event gelesen werden.");
                    }
                }
            }, false);
        });
    ...
    // Direct copy-paste (Ctrl+V) listener: Right click -> "Kopieren" on Google Photos 
    // and Ctrl+V inside the defect's description text box to instantly upload!
    document.addEventListener('paste', function(e) {
        const target = e.target;
        if (!target || !target.id || !target.id.startsWith('desc_')) {
            return;
        }
        
        const num = target.id.replace('desc_', '');
        ...
        if (imageFile) {
            e.preventDefault(); // Stop default text pasting of image metadata/references
            
            const wrapper = document.querySelector(`.photo-upload-wrapper[data-num="${num}"]`);
            const uploadBtn = wrapper ? wrapper.querySelector('.btn-photo-upload') : null;
            if (uploadBtn) {
                showToast("Bild aus Zwischenablage erkannt, lade hoch...");
                uploadPhotoFile(num, imageFile, uploadBtn);
            }
        }
    });
