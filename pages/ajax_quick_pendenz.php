    ...
    foreach ($configs as $field => $cfg) {
        ...
        for ($i=0; $i<$count; $i++) {
            ...
            if (move_uploaded_file($tmp, $targetFile)) {
                ...
                $stmt = $db->prepare("INSERT INTO pendenz_dateien (pendenz_id, typ, pfad, mimetype, groesse, titel, is_cover, hochgeladen_von) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("isssisii", $pendenzId, $cfg['typ'], $path, $type, $size, $name, $newIsCover, $vUid);
                $stmt->execute();
                $stmt->close();
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $extra_json = json_encode([
            ...
            'raum_id' => $raum_id,
            'plan_pins' => json_decode($_POST['plan_pins'] ?? '[]', true),
            'source' => 'quick_capture'
        ]);

        $plan_id = (int)($_POST['plan_id'] ?? 0) ?: null;
        $pin_x = isset($_POST['pin_x']) && $_POST['pin_x'] !== '' ? (float)str_replace(',', '.', $_POST['pin_x']) : null;
        $pin_y = isset($_POST['pin_y']) && $_POST['pin_y'] !== '' ? (float)str_replace(',', '.', $_POST['pin_y']) : null;

        // SNAPSHOT HANDLING
        $pendingSnapshotInsert = null;
        $snapData = $_POST['plan_snapshot_data'] ?? '';
        if (!empty($snapData) && $snapData !== 'DELETE') {
            ...
            if ($imgData) {
                ...
                if (file_put_contents($snapPath, $imgData)) {
                    $pendingSnapshotInsert = 'uploads/pendenzen/' . $snapName;
                }
            }
        }
        ...
        // Snapshot zuerst (wie in pendenz_neu.php)
        if ($pendingSnapshotInsert) {
            $snapEsc = $mysqli->real_escape_string($pendingSnapshotInsert);
            $mysqli->query("INSERT INTO pendenz_dateien (pendenz_id, typ, pfad, mimetype, groesse, titel, is_cover, sort_index) 
                            VALUES ($newId, 'image', '$snapEsc', 'image/jpeg', 0, 'Plan-Ausschnitt', 0, 9999)");
        }
