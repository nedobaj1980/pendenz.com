...
try {
    ...
    // Passwort muss NULL sein dürfen für Einladungs-Workflow
    $mysqli->query("ALTER TABLE benutzer MODIFY COLUMN passwort varchar(255) NULL DEFAULT NULL");
} catch (Throwable $e) {}
...
$hasType = !empty($types);
$hasStatuses = !empty($statusesMap);

/* ===== Mehrere Personen-Typen ===== */
function ensure_benutzer_personentypen_table(mysqli $db): void {
    static $done = false;
    if ($done) return;
    $done = true;
    $sql = "CREATE TABLE IF NOT EXISTS benutzer_personentypen (
        id INT NOT NULL AUTO_INCREMENT,
        benutzer_id INT NOT NULL,
        person_type_id INT NOT NULL,
        person_status_id INT NULL DEFAULT NULL,
        is_primary TINYINT(1) NOT NULL DEFAULT 0,
        sort_order INT NOT NULL DEFAULT 0,
        created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        PRIMARY KEY (id),
        KEY idx_bpt_benutzer (benutzer_id),
        KEY idx_bpt_type (person_type_id),
        KEY idx_bpt_status (person_status_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
    try { $db->query($sql); } catch (Throwable $e) {}
}
function fetch_person_type_memberships(mysqli $db, int $benutzerId): array {
    ensure_benutzer_personentypen_table($db);
    if ($benutzerId <= 0) return [];
    $rows = [];
    try {
        $sql = "SELECT bpt.*, pt.name AS type_name, ps.name AS status_name
                ...
                WHERE bpt.benutzer_id = ?
                ORDER BY bpt.is_primary DESC, bpt.sort_order ASC, bpt.id ASC";
        $st = $db->prepare($sql);
        $st->bind_param('i', $benutzerId);
        $st->execute();
        $rs = $st->get_result();
...
function normalize_person_memberships_input($typeIds, $statusIds, ?int $primaryTypeId = null, ?int $primaryStatusId = null): array {
    $typeIds = is_array($typeIds) ? $typeIds : [];
    $statusIds = is_array($statusIds) ? $statusIds : [];
    $rows = [];
    if (($primaryTypeId ?? 0) > 0) {
        $rows[] = [
            'person_type_id' => (int)$primaryTypeId,
            'person_status_id' => ($primaryStatusId ?? 0) > 0 ? (int)$primaryStatusId : null,
            'is_primary' => 1,
        ];
    }
    $n = max(count($typeIds), count($statusIds));
    for ($i = 0; $i < $n; $i++) {
        $tid = (int)($typeIds[$i] ?? 0);
        $sid = (int)($statusIds[$i] ?? 0);
        if ($tid <= 0) continue;
        $exists = false;
        foreach ($rows as $row) {
            if ((int)$row['person_type_id'] === $tid && (int)($row['person_status_id'] ?? 0) === $sid) {
                $exists = true;
                break;
            }
        }
        if (!$exists) {
            $rows[] = [
                'person_type_id' => $tid,
                'person_status_id' => $sid > 0 ? $sid : null,
                'is_primary' => 0,
            ];
        }
    }
...
function sync_person_type_memberships(mysqli $db, int $benutzerId, array $rows): void {
    ensure_benutzer_personentypen_table($db);
    if ($benutzerId <= 0) return;
    try {
        $db->begin_transaction();
        $del = $db->prepare("DELETE FROM benutzer_personentypen WHERE benutzer_id = ?");
        $del->bind_param('i', $benutzerId);
        $del->execute();
        $del->close();
        ...
        if (!empty($rows)) {
            $ins = $db->prepare("INSERT INTO benutzer_personentypen (benutzer_id, person_type_id, person_status_id, is_primary, sort_order) VALUES (?,?,?,?,?)");
            foreach ($rows as $idx => $row) {
                $tid = (int)($row['person_type_id'] ?? 0);
                if ($tid <= 0) continue;
                $sid = (int)($row['person_status_id'] ?? 0);
                $isPrimary = $idx === 0 ? 1 : (int)($row['is_primary'] ?? 0);
                $sidBind = $sid > 0 ? $sid : null;
                $ins->bind_param('iiiii', $benutzerId, $tid, $sidBind, $isPrimary, $sort);
                $ins->execute();
                if ($primaryTypeId === null) {
                    $primaryTypeId = $tid;
                    $primaryStatusId = $sid > 0 ? $sid : null;
                }
                $sort += 10;
            }
            $ins->close();
        ...
        $has = table_has_columns($db, 'benutzer', ['person_type_id','person_status_id']);
        if (in_array('person_type_id', $has, true) || in_array('person_status_id', $has, true)) {
            $sql = "UPDATE benutzer SET ";
            $parts = [];
            $types = '';
            $vals = [];
            if (in_array('person_type_id', $has, true)) {
                $parts[] = "person_type_id=?";
                $types .= 'i';
                $vals[] = $primaryTypeId;
            }
            if (in_array('person_status_id', $has, true)) {
                $parts[] = "person_status_id=?";
                $types .= 'i';
                $vals[] = $primaryStatusId;
            }
...
ensure_benutzer_personentypen_table($mysqli);
...
if (!$user && $canViewOthers) {
    $rs = $mysqli->query("SELECT * FROM benutzer ORDER BY id ASC LIMIT 1");
    if ($rs) {
        $user = $rs->fetch_assoc();
        $rs->close();
        if ($user) {
...
if (!$user) {
    ...
    exit('Benutzer nicht gefunden. Bitte erneut einloggen oder einen gültigen Benutzer öffnen.');
}

$userTypeMemberships = fetch_person_type_memberships($mysqli, (int)$viewId);
...
function firmen_select_all(mysqli $db): array {
    $sqlFull = "SELECT id,name,adresse,ort,telefon,email,website,logo FROM firmen ORDER BY name";
    try {
    ...
    $out=[]; try {
        if ($res=$db->query("SELECT id,name FROM firmen ORDER BY name")) {
            while($r=$res->fetch_assoc()){
                $r += ['adresse'=>null,'ort'=>null,'telefon'=>null,'email'=>null,'website'=>null,'logo'=>null];
                $out[]=$r;
            }
            $res->close();
        }
...
// Master-Sync: Wenn firma_id (oder Name) gesetzt ist, ziehen wir die Daten aus der Firmen-Tabelle
if ($user) {
    $f_found = null;
    $fid = (int)($user['firma_id'] ?? 0);
    if ($fid > 0 && isset($firmen_by_id[$fid])) {
        $f_found = $firmen_by_id[$fid];
    } elseif ($fid == 0 && !empty($user['firma_name'])) {
        foreach ($firmen as $f) {
            if (trim(mb_strtolower($f['name'] ?? '')) === trim(mb_strtolower($user['firma_name'] ?? ''))) {
                $f_found = $f;
                break;
            }
        }
    ...
    if ($f_found) {
        $user['firma_id']      = $f_found['id'];
        $user['firma_name']    = $f_found['name'];
        $user['firma_adresse'] = $f_found['adresse'] ?? '';
        $user['firma_ort']     = $f_found['ort']     ?? '';
        $user['firma_telefon'] = $f_found['telefon'] ?? '';
        $user['firma_email']   = $f_found['email']   ?? '';
        $user['firma_website'] = $f_found['website'] ?? '';
    }
}
...
function shrink_image_under_limit(string $pathOrUrl, int $maxW, int $maxH, int $maxBytes, string $PREFIX): ?string {
    ...
    // imagedestroy($im); // Deprecated in PHP 8.x
    ...
    do{
        ...
        if ($q<55){
            ...
            // imagedestroy($work); // Deprecated in PHP 8.x
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    elseif ($action === 'create_user') {
      ...
      if ($new_name === '') throw new Exception("Name ist ein Pflichtfeld.");

      if ($new_email === '') {
          if ($new_telefonnummer !== '') {
              // Auto-generate placeholder email from phone number
              $cleanPhone = preg_replace('/[^0-9]+/', '', $new_telefonnummer);
              $new_email = $cleanPhone . '@no-email.pendenz.com';
          } else {
              throw new Exception("Entweder E-Mail oder Telefonnummer muss angegeben werden.");
          }
      }
      ...
      if ($exists) {
          if (strpos($new_email, '@no-email.pendenz.com') !== false) {
              throw new Exception("Diese Telefonnummer ist bereits registriert.");
          } else {
              throw new Exception("Diese E-Mail ist bereits vergeben.");
          }
      }

      $new_adresse       = trim($_POST['new_adresse'] ?? '') ?: null;
      $new_geburtsdatum  = (!empty($_POST['new_geburtsdatum'])) ? $_POST['new_geburtsdatum'] : null;
      $new_heimatland    = trim($_POST['new_heimatland'] ?? '') ?: null;
      $new_position      = trim($_POST['new_position'] ?? '') ?: null;
      $new_beruf         = trim($_POST['new_beruf'] ?? '') ?: null;
      ...
      $new_extra_person_type_ids   = $_POST['new_extra_person_type_id'] ?? [];
      $new_extra_person_status_ids = $_POST['new_extra_person_status_id'] ?? [];
      $newPersonMemberships = normalize_person_memberships_input($new_extra_person_type_ids, $new_extra_person_status_ids, $new_person_type_id, $new_person_status_id);
      ...
      // Automatischer Business-Typ basierend auf dem Namen des Personen-Typs
      $newBT = 'standard';
      $personTypeId = (int)($_POST['new_person_type_id'] ?? 0);
      if ($personTypeId > 0) {
          $ptRes = $mysqli->query("SELECT name FROM person_types WHERE id = $personTypeId");
          if ($ptRow = $ptRes->fetch_assoc()) {
              $pName = strtolower($ptRow['name']);
              if (strpos($pName,'mieter')!==false || strpos($pName,'kunde')!==false || strpos($pName,'interessent')!==false) $newBT = 'mieter';
              elseif (strpos($pName,'handwerker')!==false || strpos($pName,'unternehmer')!==false || strpos($pName,'partner')!==false || strpos($pName,'lieferant')!==false) $newBT = 'handwerker';
          }
      }
      ...
      if (in_array('business_type',$benutzerCols,true)) { $cols[] = 'business_type'; $types .= 's'; $vals[] = $newBT; }
      ...
      // business_type wurde oben bereits in $newBT berechnet und zu $cols/$vals hinzugefügt.
      ...
      sync_person_type_memberships($mysqli, $newId, $newPersonMemberships);
    ...
    elseif ($action === 'save_profile') {
      ...
      foreach ($FIELDS as $key => $label) {
        $data[$key] = ($key === 'geburtsdatum') ? ((!empty($_POST[$key])) ? $_POST[$key] : null) : trim($_POST[$key] ?? '');
        $visKey = 'vis_'.$key;
      ...
      $extra_person_type_ids   = $_POST['extra_person_type_id'] ?? [];
      $extra_person_status_ids = $_POST['extra_person_status_id'] ?? [];
      $personMemberships = normalize_person_memberships_input($extra_person_type_ids, $extra_person_status_ids, $upd_type_id, $upd_status_id);

      $firmaSelectId = (int)($_POST['firma_select_id'] ?? 0);
      // Wir speichern immer das, was im Formular steht. 
      // Die PHP-Logik hier dient nur noch als Sicherheitsnetz, falls Felder im POST fehlen würden.
      if ($firmaSelectId > 0) {
        $f = firmen_by_id($mysqli, $firmaSelectId);
        if ($f) {
          // Falls Felder im Formular leer abgeschickt wurden, nehmen wir die Stammdaten
          if (empty($data['firma_name']))    $data['firma_name']    = $f['name']    ?? '';
          if (empty($data['firma_adresse'])) $data['firma_adresse'] = $f['adresse'] ?? '';
          if (empty($data['firma_ort']))     $data['firma_ort']     = $f['ort']     ?? '';
          if (empty($data['firma_telefon'])) $data['firma_telefon'] = $f['telefon'] ?? '';
          if (empty($data['firma_email']))   $data['firma_email']   = $f['email']   ?? '';
          if (empty($data['firma_website'])) $data['firma_website'] = $f['website'] ?? '';
        }
      }
...
$pb_w = max(80, min(400, $pb_w));  $profileVis['profilbild_max_size'] = $pb_w;
      ...
      if (($data['name'] ?? '') === "") {
        throw new Exception("Name ist ein Pflichtfeld.");
      }

      if (($data['email'] ?? '') === "") {
          if (($data['telefonnummer'] ?? '') !== '') {
              // Auto-generate placeholder email from phone number
              $cleanPhone = preg_replace('/[^0-9]+/', '', $data['telefonnummer']);
              $data['email'] = $cleanPhone . '@no-email.pendenz.com';
          } else {
              throw new Exception("Entweder E-Mail oder Telefonnummer muss angegeben werden.");
          }
      }
      ...
      $sql = "
        UPDATE benutzer SET
          name=?, email=?, rolle=?,
          telefonnummer=?, adresse=?, ort=?,
          firma_name=?, firma_adresse=?, firma_ort=?, firma_telefon=?, firma_email=?, firma_website=?,
          geburtsdatum=?, heimatland=?, position=?, beruf=?,
          kontaktweg=?,
          profilbild=?, firmenlogo=?, titelbild=?,
          profile_vis=?, firma_id=?, profile_updated_at=NOW()";
      $types = 'sssssssssssssssssssssi'; // 22 items
      $vals = [
        $data['name'], $data['email'], $finalRole,
        $data['telefonnummer'], $data['adresse'], $data['ort'],
        $data['firma_name'], $data['firma_adresse'], $data['firma_ort'], $data['firma_telefon'], $data['firma_email'], $data['firma_website'],
        $data['geburtsdatum'], $data['heimatland'], $data['position'], $data['beruf'],
        trim($_POST['kontaktweg'] ?? 'email'),
        $profilbild, $firmenlogo, $titelbild,
        $visJson,
        $firmaSelectId > 0 ? $firmaSelectId : null
      ];
      ...
      sync_person_type_memberships($mysqli, $targetId, $personMemberships);
    ...
    $msg = $e->getMessage();
    $flash = "❌ " . (function_exists('translate_db_error') ? translate_db_error($msg) : $msg);
  }
}
?>
<style>
...
<div class="container" id="profile-root" data-prefix="<?= h($PREFIX) ?>">
  ...
  <?php
    $isOwnProfile = ((int)$viewId === (int)$sid);
    $primaryMembership = $userTypeMemberships[0] ?? null;
    $extraMemberships = array_slice($userTypeMemberships, 1);
    $editPersonTypeId = (int)($user['person_type_id'] ?? ($primaryMembership['person_type_id'] ?? 0));
    $editPersonStatusId = (int)($user['person_status_id'] ?? ($primaryMembership['person_status_id'] ?? 0));
  ?>

  <div class="card" id="edit-profile-section">
    <div style="display:flex; justify-content:space-between; align-items:center; gap:16px; flex-wrap:wrap; margin-bottom:18px;">
      <div>
        <h2 style="margin:0; font-size:24px; font-weight:800; color:#0f172a;">✏️ <?= $isOwnProfile ? 'Mein Profil bearbeiten' : 'Profil bearbeiten: '.h($user['name'] ?? ('Benutzer #'.$viewId)) ?></h2>
        <div style="margin-top:6px; color:#64748b; font-size:13px;">Hier kannst du dein Profil oder andere Profile direkt bearbeiten.</div>
      </div>
      <?php if (!$isOwnProfile): ?>
        <a href="<?= h($PREFIX) ?>pages/profil.php" class="btn-outline btn-small">Zu meinem Profil</a>
      <?php endif; ?>
    </div>

    <form method="post" autocomplete="on" enctype="multipart/form-data" id="edit-profile-form">
      <?= benutzer_csrf_input() ?>
        ...
        <div class="modern-grid">
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              ...
              <button type="button" class="icon-toggle" data-for="name" title="<?= h($LEVEL_LABEL[$profileVis['name'] ?? 'private']) ?>"><?= $LEVEL_ICON[$profileVis['name'] ?? 'private'] ?></button>
            </div>
            <input type="hidden" id="vis_name" name="vis_name" value="<?= h($profileVis['name'] ?? 'private') ?>">
          </div>
          <div class="modern-field">
            <label>E-Mail (oder Telefonnummer)</label>
            <div class="modern-input-group">
              <input type="text" name="email" value="<?= h($user['email'] ?? '') ?>">
              <button type="button" class="icon-toggle" data-for="email" title="<?= h($LEVEL_LABEL[$profileVis['email'] ?? 'private']) ?>"><?= $LEVEL_ICON[$profileVis['email'] ?? 'private'] ?></button>
            </div>
            <input type="hidden" id="vis_email" name="vis_email" value="<?= h($profileVis['email'] ?? 'private') ?>">
          </div>
          ...
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              <select name="rolle">
                <?php $editAllowedRoles = ['gast','benutzer','admin']; if ($currentRole === 'superadmin') $editAllowedRoles[] = 'superadmin'; ?>
                <?php foreach ($editAllowedRoles as $rr): ?>
                  <option value="<?= h($rr) ?>" <?= (($user['rolle'] ?? 'benutzer') === $rr ? 'selected' : '') ?>><?= h($rr) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          ...
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              <select name="vorgangsart_id">
                <option value="">— Keine (Standard) —</option>
                <?php foreach ($vorgangsarten as $va): ?>
                  <option value="<?= (int)$va['id'] ?>" <?= (int)($user['vorgangsart_id'] ?? 0) === (int)$va['id'] ? 'selected' : '' ?>><?= h($va['name']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
      ...
      <div class="modern-form-section">
        ...
        <div class="modern-grid">
          <div class="modern-field">
            <label>Projekt</label>
            <div class="modern-input-group">
              <select id="edit_projekt_id" name="projekt_id">
          ...
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              <select id="edit_objekt_id" name="objekt_id" disabled>
          ...
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              <select id="edit_wohnung_id" name="wohnung_id" disabled>
          ...
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              <select id="person_type_id" name="person_type_id">
                <option value="">— auswählen —</option>
                <?php foreach ($types as $t): ?>
                  <option value="<?= (int)$t['id'] ?>" <?= $editPersonTypeId === (int)$t['id'] ? 'selected' : '' ?>><?= h($t['name']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          <div class="modern-field">
            <label>Status</label>
            <div class="modern-input-group">
              <select id="person_status_id" name="person_status_id" data-selected="<?= (int)$editPersonStatusId ?>" disabled>
          ...
          <div class="modern-field" style="grid-column:1 / -1;">
            <label>Weitere Personen/Sub-Typen & Status</label>
            <div id="personTypeMembershipRows" style="display:flex; flex-direction:column; gap:10px;">
              <?php foreach ($extraMemberships as $m): ?>
                <div class="person-membership-row" style="display:grid; grid-template-columns:minmax(220px,1fr) minmax(220px,1fr) auto; gap:10px; align-items:center;">
                  <select name="extra_person_type_id[]" class="person-type-extra">
                    <option value="">— Typ wählen —</option>
                    <?php foreach ($types as $t): ?>
                      <option value="<?= (int)$t['id'] ?>" <?= (int)$m['person_type_id'] === (int)$t['id'] ? 'selected' : '' ?>><?= h($t['name']) ?></option>
                    <?php endforeach; ?>
                  </select>
                  <select name="extra_person_status_id[]" class="person-status-extra">
                    <option value="<?= (int)($m['person_status_id'] ?? 0) ?>"><?= h($m['status_name'] ?? '— auswählen —') ?></option>
                  </select>
                  <button type="button" class="btn btn-sm remove-membership-row">Entfernen</button>
                </div>
              <?php endforeach; ?>
            </div>
            <div style="margin-top:10px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
              <button type="button" id="addPersonTypeMembership" class="btn btn-sm">+ Weiteren Personen/Sub-Typ hinzufügen</button>
              <span style="font-size:12px; color:#64748b;">Oben bleibt der Haupt-Personen-Typ. Hier kannst du weitere Typen mit Status ergänzen.</span>
            </div>
          </div>
          <?php endif; ?>
        </div>
      ...
      <div class="modern-form-section">
        <div class="modern-form-title">📞 Weitere Kontaktdaten</div>
        <div class="modern-grid">
          <?php foreach ([['telefonnummer','Telefon'],['adresse','Adresse'],['ort','Ort'],['geburtsdatum','Geburtstag','date'],['heimatland','Heimatland'],['position','Position'],['beruf','Beruf']] as $cf):
              [$id,$lbl,$typ] = [$cf[0],$cf[1],$cf[2] ?? 'text']; ?>
            <div class="modern-field">
              <label><?= h($lbl) ?></label>
              <div class="modern-input-group">
                <input type="<?= h($typ) ?>" id="f_<?= h($id) ?>" name="<?= h($id) ?>" value="<?= h($user[$id] ?? '') ?>">
                <button type="button" class="icon-toggle" data-for="<?= h($id) ?>" title="<?= h($LEVEL_LABEL[$profileVis[$id] ?? 'private']) ?>"><?= $LEVEL_ICON[$profileVis[$id] ?? 'private'] ?></button>
              </div>
              <input type="hidden" id="vis_<?= h($id) ?>" name="vis_<?= h($id) ?>" value="<?= h($profileVis[$id] ?? 'private') ?>">
            </div>
          <?php endforeach; ?>
          <div class="modern-field">
            ...
            <div class="modern-input-group">
              <select name="kontaktweg">
                <?php foreach (['email'=>'📧 E-Mail','telefon'=>'📞 Telefon','whatsapp'=>'💬 WhatsApp','sms'=>'📱 SMS','post'=>'📮 Postweg'] as $kv=>$kl): ?>
                  <option value="<?= h($kv) ?>" <?= (($user['kontaktweg'] ?? 'email') === $kv ? 'selected' : '') ?>><?= h($kl) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
      ...
      <div class="modern-form-section">
        <div class="modern-form-title">🏢 Firma</div>
        <?php if (!empty($firmen)): ?>
          <div class="modern-field" style="margin-bottom:16px;">
            <label>Firma (aus Stammdaten)</label>
            <div class="modern-input-group">
              <select id="firma_select_id" name="firma_select_id">
                <option value="">— auswählen —</option>
                <?php foreach ($firmen as $f): ?>
                  <option value="<?= (int)$f['id'] ?>" <?= (($user['firma_name'] ?? '') === ($f['name'] ?? '') ? 'selected' : '') ?>><?= h($f['name'] ?: ('Firma #'.(int)$f['id'])) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
        <?php endif; ?>
        <div class="modern-grid">
          <?php foreach ([['firma_name','Firma – Name'],['firma_adresse','Firma – Adresse'],['firma_ort','Firma – Ort'],['firma_telefon','Firma – Telefon'],['firma_email','Firma – E-Mail','email'],['firma_website','Firma – Website','url']] as $cf):
              [$id,$lbl,$typ] = [$cf[0],$cf[1],$cf[2] ?? 'text']; ?>
            <div class="modern-field">
              <label><?= h($lbl) ?></label>
              <div class="modern-input-group">
      ...
      <div style="padding:20px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;">
        <div style="font-size:13px; color:#64748b;">Benutzer-ID: <strong>#<?= (int)$viewId ?></strong></div>
        <div style="display:flex; gap:10px; align-items:center;">
          <a href="<?= h($PREFIX) ?>pages/profil.php?view=<?= (int)$viewId ?>" class="btn-outline btn-small">Neu laden</a>
          <button class="btn-premium" type="submit" style="padding:14px 34px; font-size:16px; border:none; cursor:pointer;">Speichern</button>
        </div>
      </div>
    </form>
  </div>
    ...
    <form method="post" autocomplete="on" enctype="multipart/form-data" id="create-user-form">
        ...
        <div class="modern-grid">
          ...
          <?php if (!empty($types)): ?>
            <div class="modern-field" id="wrap_person_type_inner">
              <label>Benutzer-Typ*</label>
              <div class="modern-input-group">
                <select id="new_person_type_id" name="new_person_type_id" required>
                  <option value="">— auswählen —</option>
                  <?php foreach ($types as $t): ?>
      ...
      <div class="modern-form-section">
        ...
        <div class="modern-grid">
          <?php
            $createFields = [
              ['new_telefonnummer','Telefon (Pflicht)','tel', true],
              ['new_adresse','Adresse'],
              ['new_ort','Ort'],
            ...
            foreach ($createFields as $cf) {
              [$id,$lbl,$typ,$req] = [$cf[0],$cf[1],$cf[2] ?? 'text', $cf[3] ?? false];
              $base = str_replace('new_','',$id);
              $lvl='private';
          ?>
            <div class="modern-field">
              <label><?= h($lbl) ?><?= $req ? '*' : '' ?></label>
              <div class="modern-input-group">
                <input type="<?= h($typ) ?>" id="<?= h($id) ?>" name="<?= h($id) ?>" <?= $req ? 'required' : '' ?>>
  ...
  <div class="card">
    ...
    $sql_all = "SELECT u.*, 
               ...
               LEFT JOIN person_statuses ps ON ps.id = u.person_status_id
               ORDER BY u.name ASC";
    ...
    foreach($allUsers as $u) {
        ...
        $typeName   = strtolower($u['type_name']   ?? '');
        $statusName = strtolower($u['status_name'] ?? '');
        $bType      = $u['business_type'] ?? 'standard';
        $role       = $u['rolle'] ?? 'benutzer';

        // Logik: Zuerst Unternehmer (Handwerker)
        if ($bType === 'handwerker' || $typeName === 'handwerker' || $typeName === 'unternehmer') {
            $groups['unternehmer']['users'][] = $u;
        } 
        // Mieter Logik anhand der neuen Status-Namen
        elseif ($typeName === 'mieter' || $typeName === 'kunde') {
            if ($statusName === 'mieter' || $statusName === 'aktiv') {
                $groups['mieter']['users'][] = $u;
            } elseif ($statusName === 'vormieter' || $statusName === 'gekündigt' || $statusName === 'ehemalig') {
                $groups['vormieter']['users'][] = $u;
            } elseif ($statusName === 'interessent' || $statusName === 'bewerber' || $statusName === 'mietinteressent') {
                $groups['bewerber']['users'][] = $u;
            } else {
                $groups['mieter']['users'][] = $u; // Default Mieter
            }
        } 
        // Mitarbeiter / Admin
        elseif ($bType === 'standard' || $role === 'admin' || $role === 'superadmin' || $typeName === 'mitarbeiter') {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div style="font-size:10px; color:#64748b; margin-top:2px;">
                                            <?= h($u['type_name'] ?? $u['business_type'] ?? '') ?>
                                            <?php if(!empty($u['status_name'])): ?> · <span style="color:#3b82f6;"><?= h($u['status_name']) ?></span><?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(selEdit){
    selEdit.addEventListener('change', function(){
      ...
      if(!id || !window.FIRMEN_DATA) return;
      const f = window.FIRMEN_DATA[id]; if(!f) return;
      const setField = (name,val)=>{ const el=document.getElementById('f_'+name); if(el) el.value = val||''; };
      setField('firma_name',    f.name);
      setField('firma_adresse', f.adresse);
      setField('firma_ort',     f.ort);
      setField('firma_telefon', f.telefon);
      setField('firma_email',   f.email);
      setField('firma_website', f.website);
 ...
 */
function handleTaxonomyChange(typeId, typeName) {
    ...
    // Logik basierend auf dem Namen des Typs
    if (name.includes('mieter') || name.includes('kunde') || name.includes('interessent')) {
        roleSel.value = 'gast';
        // Smart-Select Object "10_Mietsache"
        if (objSel) {
            for (let i = 0; i < objSel.options.length; i++) {
                if (objSel.options[i].text.toLowerCase().includes('10_mietsache')) {
    ...
    else if (name.includes('handwerker') || name.includes('unternehmer') || name.includes('partner') || name.includes('lieferant')) {
        roleSel.value = 'gast';
        if (wrapBkp) wrapBkp.style.display = 'block';
    } 
    else {
        // Default: Mitarbeiter / Team
...
if (typeEdit && statEdit) {
  const initialEditStatus = parseInt(statEdit.getAttribute('data-selected') || statEdit.value || '0', 10) || null;
  fillStatuses(statEdit, parseInt(typeEdit.value || '0',10), initialEditStatus);
...
if (typeNew && statNew) {
  typeNew.addEventListener('change', ()=>{
    ...
    const tname = typeNew.options[typeNew.selectedIndex].text;
    fillStatuses(statNew, tid, null);
    handleTaxonomyChange(tid, tname);
  });
}
</script>
...
<script>
function createMembershipRowHtml(prefix) {
  const wrap = document.createElement('div');
  wrap.className = 'person-membership-row';
  wrap.style.display = 'grid';
  wrap.style.gridTemplateColumns = 'minmax(220px,1fr) minmax(220px,1fr) auto';
  wrap.style.gap = '10px';
  wrap.style.alignItems = 'center';

  const typeSel = document.createElement('select');
  typeSel.name = prefix + '_person_type_id[]';
  typeSel.className = prefix === 'new_extra' ? 'person-type-extra-new' : 'person-type-extra';
  const statusSel = document.createElement('select');
  statusSel.name = prefix + '_person_status_id[]';
  statusSel.className = prefix === 'new_extra' ? 'person-status-extra-new' : 'person-status-extra';
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'btn btn-sm remove-membership-row';
  btn.textContent = 'Entfernen';
  const defType = document.createElement('option');
  defType.value = '';
  defType.textContent = '— Typ wählen —';
  typeSel.appendChild(defType);
  <?php foreach ($types as $t): ?>
  {
    const o = document.createElement('option');
    o.value = '<?= (int)$t['id'] ?>';
    o.textContent = '<?= h($t['name']) ?>';
    typeSel.appendChild(o);
  }
  <?php endforeach; ?>
  fillStatuses(statusSel, 0, null);
  typeSel.addEventListener('change', ()=>fillStatuses(statusSel, parseInt(typeSel.value || '0', 10), null));
...
document.addEventListener('click', function(e){
  if (e.target && e.target.id === 'addPersonTypeMembership') {
    const host = document.getElementById('personTypeMembershipRows');
    if (host) host.appendChild(createMembershipRowHtml('extra'));
  }
  if (e.target && e.target.id === 'addNewPersonTypeMembership') {
    const host = document.getElementById('newPersonTypeMembershipRows');
    if (host) host.appendChild(createMembershipRowHtml('new_extra'));
  }
  if (e.target && e.target.classList.contains('remove-membership-row')) {
    const row = e.target.closest('.person-membership-row');
    if (row) row.remove();
  }
});
document.querySelectorAll('.person-membership-row').forEach(row=>{
  const t = row.querySelector('.person-type-extra, .person-type-extra-new');
  const s = row.querySelector('.person-status-extra, .person-status-extra-new');
  if (t && s) {
    fillStatuses(s, parseInt(t.value || '0', 10), s.value || null);
    t.addEventListener('change', ()=>fillStatuses(s, parseInt(t.value || '0', 10), null));
  }
});
</script>
