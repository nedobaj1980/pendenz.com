<?php
// pages/pendenzen.php
ob_start(); // Output-Buffer: verhindert "headers already sent" bei AJAX-Actions
// Stand: 2025-10-02
...
$ASSET  = rtrim($PREFIX,'/').'/'; 
$flash  = "";
// Der <script>-Block wurde nach unten (nach header.php) verschoben.
...
/* --- SCHNELL-ERFASSUNG (Quick Capture) --- */
...
.qc-header h2 { margin: 0; font-size: 20px; font-weight: 800; }
.qc-close { background: rgba(255,255,255,0.2); border: none; color: white; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; font-size: 20px; line-height: 1; display: flex; align-items: center; justify-content: center; }

.qc-body { padding: 24px; overflow-y: auto; flex: 1; }
.qc-photo-section { margin-bottom: 24px; }
.qc-photo-drop {
    border: 2px dashed #e2e8f0; border-radius: 20px; padding: 25px;
    text-align: center; background: #f8fafc; cursor: pointer; transition: all 0.2s;
}
.qc-photo-drop:hover { border-color: #1abc9c; background: #f0fdfa; }
.qc-photo-drop span.icon { font-size: 32px; display: block; margin-bottom: 5px; }

.qc-type-selector { display: flex; gap: 8px; margin-bottom: 24px; overflow-x: auto; padding-bottom: 5px; scrollbar-width: none; }
.qc-type-btn {
    flex: 1; min-width: 75px; aspect-ratio: 1; border-radius: 18px; border: 2px solid #f1f5f9;
...
.qc-type-btn.active { border-color: #1abc9c; background: #f0fdfa; color: #0d9488; transform: translateY(-3px); box-shadow: 0 8px 15px rgba(26, 188, 156, 0.2); }
.qc-type-btn span.emoji { font-size: 24px; }
.qc-type-btn label { font-size: 10px; font-weight: 800; cursor: pointer; text-transform: uppercase; letter-spacing: 0.3px; }

.qc-field { margin-bottom: 18px; }
.qc-field label { display: block; font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase; margin-bottom: 6px; letter-spacing: 0.05em; }
.qc-input { width: 100%; padding: 14px 16px; border-radius: 14px; border: 1.5px solid #e2e8f0; font-size: 15px; font-weight: 600; background: #f8fafc; transition: all 0.2s; }
.qc-input:focus { outline: none; border-color: #1abc9c; background: #fff; box-shadow: 0 0 0 4px rgba(26, 188, 156, 0.1); }

.qc-footer { padding: 20px 24px; background: #fff; border-top: 1px solid #f1f5f9; }
.qc-btn-save { width: 100%; padding: 18px; border-radius: 18px; background: #1abc9c; color: white; border: none; font-weight: 800; font-size: 17px; cursor: pointer; box-shadow: 0 10px 25px rgba(26, 188, 156, 0.3); transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 10px; }
.qc-btn-save:hover { background: #16a085; transform: translateY(-2px); box-shadow: 0 15px 30px rgba(26, 188, 156, 0.4); }
.qc-btn-save:active { transform: translateY(0); }

.qc-context-info { margin-bottom: 15px; background: #f1f5f9; padding: 10px 14px; border-radius: 12px; font-size: 12px; color: #475569; font-weight: 700; display: flex; align-items: center; gap: 8px; }
    ...
    .form-card.compact { padding: 18px; }
    .form-card h3 {
        margin: 0 0 18px 0;
        font-size: 15px;
        font-weight: 800;
    ...
    .form-card h3 svg { width: 20px; height: 20px; stroke: #0284c7; }
    .form-field { margin-bottom: 15px; }
    .form-field label { display: block; font-size: 11px; font-weight: 700; color: #64748b; margin-bottom: 6px; text-transform: uppercase; }
    .form-input { 
        width: 100%; 
        padding: 10px 14px; 
    ...
    .form-input:focus { border-color: #0284c7; background: #fff; outline: none; box-shadow: 0 0 0 4px rgba(2, 132, 199, 0.1); }
    
    @media (max-width: 1024px) {
        .form-step-container { grid-template-columns: 1fr; }
    }
</style>
...
<div class="container-fluid">
  ...
  <header class="hero hero-teal" style="display:flex;gap:8px;justify-content:space-between;align-items:center; border-radius: 12px; margin-bottom: 24px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
      ...
      <button class="btn btn-head" type="button" onclick="openQuickCapture()" style="background:#1abc9c; color:#fff;">⚡ Schnell-Erfassung</button>
      ...
      <a class="btn btn-head" href="vorgangsart_settings.php">⚙️ Vorgangsarten</a>
  ...
  <div id="formSection" style="<?= $editPendenz ? '' : 'display:none;' ?>">
    ...
    <?php
      // Initial-Werte für das FORMULAR (bewusst getrennt vom Tabellenfilter unten)
      // Wichtig: Die GET-Parameter projekt_id / objekt_id / wohnung_id gehören hier zur Filterleiste
      // und dürfen NICHT direkt das Formular "Neue Pendenz" steuern.
      $pidInit   = (int)($editPendenz['projekt_id'] ?? ($last['projekt_id'] ?? 0));
      $oidInit   = (int)($editPendenz['objekt_id'] ?? ($last['objekt_id'] ?? 0));
      $widInit   = (int)($editPendenz['wohnung_id'] ?? ($last['wohnung_id'] ?? 0));

      // Fallback: Falls es noch keinen "last"-Kontext gibt, Projekt aus dem aktuellen Filter übernehmen.
      // Objekt/Wohnung absichtlich NICHT aus $_GET übernehmen, damit "Haus / Gebäude" nicht am Tabellenfilter hängt.
      if ($pidInit <= 0 && !empty($_GET['projekt_id'])) {
          $pidInit = (int)$_GET['projekt_id'];
      }
        ...
        <div class="form-column">
            ...
            <div class="form-card compact">
               <h3 style="color: #0ea5e9;">
                 ...
                 1. Express-Setup (Typ & Ort)
               </h3>
               
               <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                 <div class="form-field">
                   <label>Vorgangsart*</label>
                   <select name="vorgangsart_id" id="vorgangsart_id" class="form-input" required>
                      <option value="">— bitte wählen —</option>
                      <?php 
                        $rsA = $mysqli->query("SELECT id, name, allowed_business_types FROM pendenzen_arten WHERE is_active=1 ORDER BY sort_order, name");
                        if($rsA) while($a=$rsA->fetch_assoc()): ?>
                          <option value="<?= (int)$a['id'] ?>" <?= (int)$a['id']===(int)($editPendenz['vorgangsart_id']??0)?'selected':'' ?> data-allowed-types="<?= h($a['allowed_business_types']??'') ?>"><?= h($a['name']) ?></option>
                      <?php endwhile; ?>
                   </select>
                 </div>
                 
                 <div class="form-field">
                   <label>Zuständig (User)*</label>
                   <select name="assignee_user_id" id="assignee_user_id" class="form-input" required>
                       <option value="">— bitte wählen —</option>
                       <?php if($pidInit): 
                          $mem=fetch_members_by_project($mysqli,$pidInit); 
                          if($mem) while($m=$mem->fetch_assoc()): 
                            $mName = $m['firma_name'] ? $m['firma_name']." (".$m['name'].")" : $m['name'];
                            $mLabel = $mName . ($m['bkp_name'] ? " [".$m['bkp_name']."]" : "");
                            // Multi-BKP logic: priority to company, then user
                            $mBkpIds = !empty($m['bkp_ids_comp']) ? $m['bkp_ids_comp'] : ($m['bkp_ids_user'] ?? '');
                       ?>
                          <option value="<?= (int)$m['id'] ?>" <?= (int)$m['id']===$assUser?'selected':'' ?> data-bkp="<?= h($mBkpIds) ?>" data-bkp-name="<?= h($m['bkp_name']??'') ?>"><?= h($mLabel) ?></option>
                       <?php endwhile; endif; ?>
                    </select>
                 </div>
               </div>
               ...
               <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 10px;">
                 ...
                 <div class="form-field">
                   ...
                   <select name="objekt_id" id="objekt_id" class="form-input">
               ...
               <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-top: 5px;">
                 <div class="form-field">
                   <label style="font-size: 9px;">Ebene</label>
                   <select name="fs_branch" id="fs_branch" class="form-input" style="padding: 6px 10px; font-size: 12px;">
                      <option value="10_Mietsache" selected>10_Mietsache</option>
                      <option value="05_Allgemein">05_Allgemein</option>
                      <option value="06_Tiefgarage">06_Tiefgarage</option>
                   </select>
                 </div>
                 <div class="form-field" id="unit_select_wrap">
                   <label style="font-size: 9px;">Einheit</label>
                   <select name="wohnung_id" id="wohnung_id" class="form-input" style="padding: 6px 10px; font-size: 12px;">
                      <option value="" data-path="">— alle —</option>
                 ...
                 <div class="form-field">
                   <label style="font-size: 9px;">Raum</label>
                   <select name="raum_id" id="raum_id" class="form-input" style="padding: 6px 10px; font-size: 12px;">
                      <option value="">— kein —</option>
                      <?php if($widInit){ $rsR = $mysqli->query("SELECT id, name FROM raeume WHERE wohnung_id=$widInit ORDER BY sort_order, name"); if($rsR) while($r=$rsR->fetch_assoc()): ?>
               ...
               <div style="margin-top: 5px; opacity: 0.8;">
                   <label style="font-size: 9px; margin-bottom: 2px;">📂 Ablage-Pfad</label>
                   <input type="text" name="fs_rel_path" id="fs_rel_path" readonly class="form-input" style="background:#f1f5f9; cursor:not-allowed; border-color:transparent; font-size:10px; padding: 4px 8px;" value="<?= h($fsRelInit) ?>">
               </div>
            </div>
            ...
            <div class="form-card">
               ...
               <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 12px;">
                  <div class="form-field">
                    <label style="font-size: 9px;">Gewerke BKP</label>
                    <select id="bkp_cat_id" class="form-input" onchange="loadBkpTemplates(this.value)" style="font-size: 13px;">
                      <option value="">— wählen —</option>
                    </select>
                  </div>
                  <div class="form-field">
                    <label style="font-size: 9px;">Text-Vorlage</label>
                    <select id="bkp_tpl_id" class="form-input" onchange="applyBkpTemplate(this)" style="font-size: 13px;">
                      <option value="">— wählen —</option>
                    </select>
                  </div>
               </div>
               ...
               <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 12px;">
                 <div class="form-field">
                    <label style="font-size: 9px;">Titel-Vorlage</label>
                    <select name="kategorie_id" id="kategorie_id" class="form-input" style="font-size: 13px;">
                 ...
                 <div class="form-field">
                    <label style="font-size: 9px;">Betreff / Detail</label>
                    <select name="unterkategorie_id" id="unterkategorie_id" class="form-input" style="font-size: 13px;" disabled>
               ...
               <div class="form-field">
                 ...
                 <input type="text" name="titel" id="titel" class="form-input" placeholder="Wand beschädigt, Fenster klemmt..." required value="<?= h($editPendenz['titel'] ?? '') ?>" style="font-size: 16px; padding: 12px 16px;">
               </div>
    
               <div class="form-field">
                 <label>Beschreibung & Details</label>
                 <textarea name="langbeschreibung" id="beschreibung" class="form-input" rows="3" placeholder="Genaue Beschreibung..."><?= h($editPendenz['langbeschreibung'] ?? $editPendenz['beschreibung'] ?? '') ?></textarea>
               </div>
               
               <div class="form-field" style="background: #fff8e1; padding: 10px; border-radius: 12px; border: 1px solid #ffecb3;">
                 <label style="display:flex; align-items:center; gap:8px; cursor:pointer; margin:0;">
                    <input type="checkbox" name="is_protokoll" value="1" <?= ($editPendenz['is_protokoll'] ?? 0) ? 'checked' : '' ?>>
                    <span style="font-size: 13px; font-weight: 700; color: #92400e;">Als Protokoll-Punkt (Abnahme) markieren</span>
                 </label>
               </div>
            </div>
        </div>
        
        <!-- RIGHT COLUMN: STATUS, DATES, MEDIA -->
        <div class="form-column">
            <!-- 3. Status & Termine -->
            <div class="form-card">
               ...
               <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                 <div class="form-field">
                   ...
                   <select name="status" id="status" class="form-input">
                     <option value="offen" <?= ($editPendenz['status']??'')==='offen'?'selected':'' ?>>offen</option>
                     <option value="in arbeit" <?= ($editPendenz['status']??'')==='in arbeit'?'selected':'' ?>>in arbeit</option>
                     <option value="erledigt" <?= ($editPendenz['status']??'')==='erledigt'?'selected':'' ?>>erledigt</option>
                     <option value="abgeschlossen" <?= ($editPendenz['status']??'')==='abgeschlossen'?'selected':'' ?>>abgeschlossen</option>
                     <option value="storniert" <?= ($editPendenz['status']??'')==='storniert'?'selected':'' ?>>storniert</option>
                   </select>
                 </div>
                 <div class="form-field">
                   <label>Priorität</label>
                   <select name="wichtigkeit" id="prio" class="form-input">
                     <option value="0" <?= (int)($editPendenz['wichtigkeit']??0)==0?'selected':'' ?>>0 - Niedrig</option>
                     <option value="1" <?= (int)($editPendenz['wichtigkeit']??0)==1?'selected':'' ?>>1 - Normal</option>
                     <option value="2" <?= (int)($editPendenz['wichtigkeit']??0)==2?'selected':'' ?>>2 - Hoch</option>
                     <option value="3" <?= (int)($editPendenz['wichtigkeit']??0)==3?'selected':'' ?>>3 - Sofort!</option>
                   </select>
                 </div>
               </div>
               ...
               <div class="form-field">
                 ...
                 <input type="text" name="dauer" id="dauer" class="form-input date-calc-trigger" value="<?= h($editPendenz['dauer'] ?? '0 Tage') ?>">
               </div>
               
               <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                 <div class="form-field">
                   <label style="font-size:9px;">Start</label>
                   <input type="date" name="startdatum" id="startdatum" class="form-input date-calc-trigger" value="<?= h($editPendenz['startdatum'] ?? '') ?>" style="font-size:13px;">
                 </div>
                 <div class="form-field">
                   <label style="font-size:9px;">Fällig</label>
                   <input type="date" name="enddatum" id="enddatum" class="form-input date-calc-trigger" value="<?= h($editPendenz['enddatum'] ?? '') ?>" style="font-size:13px;">
                 </div>
               </div>
            </div>
            ...
            <div class="form-card">
               ...
               <div class="form-field">
                 ...
                 <div class="qc-photo-drop" style="padding: 15px;" onclick="document.getElementById('bilder_input').click()">
                    <span class="icon">📁</span>
                    <div id="file_status" style="font-size:11px; font-weight:700; color:#64748b;">Klicken zum Hochladen</div>
                 </div>
                 <input type="file" name="bilder[]" id="bilder_input" multiple accept="image/*, application/pdf" style="display:none;" onchange="document.getElementById('file_status').innerText = this.files.length + ' Dateien gewählt'">
               </div>
               ...
               <button type="submit" name="action" value="save" class="btn btn-teal" style="width:100%; padding: 18px; font-size: 18px; margin-top: 15px; font-weight: 800; border-radius: 18px; box-shadow: 0 10px 25px rgba(26, 188, 156, 0.3);">
                 🚀 <?= $editPendenz ? 'ÄNDERUNGEN SPEICHERN' : 'SPEICHERN & ERSTELLEN' ?>
               </button>
               <div style="text-align:center; margin-top:12px;">
                   <a href="pendenzen.php" style="color:#64748b; font-size:13px; font-weight:700; text-decoration:none;">Abbrechen</a>
               </div>
            </div>
        </div>
      </div>
  ...
  <div class="sdash-wrap" data-csrf="dummy">
    ...
    <form method="get" id="filterForm" style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; width:100%;">
      ...
      <div style="display:flex; align-items:center; gap:8px; flex:1; min-width:220px;">
        <select name="list_id" id="filter_list_id" onchange="this.form.submit()" style="background:#e0f2fe; border-color:#7dd3fc; color:#0369a1; font-weight:700; flex:1; height:44px; border-radius:8px;">
      ...
      <select name="projekt_id" id="filter_projekt_id" aria-label="Projekt filtern" onchange="updateProjectContext(this.value, 'filter'); this.form.submit()" style="flex:1; min-width:180px; height:44px; font-weight:600; border-color:#94a3b8;">
      ...
      <select name="wohnung_id" id="filter_wohnung_id" onchange="this.form.submit()" class="filter-wohnung-select" style="flex:1; min-width:180px; height:44px;">
      ...
      <select name="status" id="filter_status" onchange="this.form.submit()" style="width:140px; height:44px;">
      ...
      <select name="vorgangsart_id" id="filter_vorgangsart_id_main" onchange="this.form.submit()" style="width:160px; height:44px; border-color:#818cf8; background:#f5f3ff; font-weight:600;">
      ...
      <select name="kategorie_id" id="filter_kategorie_id" onchange="this.form.submit()" style="width:160px; height:44px;">
      ...
      <input type="text" name="q" id="q" aria-label="Suchbegriff eingeben" placeholder="Suche..." value="<?= h($q) ?>" style="flex:1.5; min-width:200px; height:44px;">
        ...
        <tr id="inlineNewRow" style="background:#f8fafc; border-top:2px solid #e2e8f0;">
          ...
          <td>
             <div style="display:flex; gap:5px; align-items:center;">
               <select id="inline_project_id" name="projekt_id" style="max-width:120px; font-size:12px;" onchange="updateProjectContext(this.value, 'inline')">
...
<div class="quick-drawer" id="quickDrawer">
    ...
    <div class="quick-field">
        <label for="qFiles">Foto hinzufügen</label>
        ...
        <input type="file" id="qFiles" name="qFiles[]" multiple accept="image/*" style="display:none;" onchange="handleQPhotos(this)">
    ...
    <div class="quick-field">
        ...
        <input type="hidden" id="qCatId" name="kategorie_id">
    </div>

    <div class="quick-field">
        <label for="qTitle">Titel / Mangel</label>
        <input type="text" id="qTitle" name="titel" class="quick-input" placeholder="Wand beschädigt, Fenster klemmt...">
...
<div id="quickCaptureOverlay" class="quick-capture-overlay">
  <div class="quick-capture-modal">
    <div class="qc-header">
      <h2>⚡ Schnell-Erfassung</h2>
      <button class="qc-close" onclick="closeQuickCapture()">✕</button>
    </div>
    <form id="qcForm" method="post" enctype="multipart/form-data" class="qc-body">
      <input type="hidden" name="projekt_id" id="qc_projekt_id">
      <input type="hidden" name="objekt_id" id="qc_objekt_id">
      <input type="hidden" name="wohnung_id" id="qc_wohnung_id">
      <input type="hidden" name="vorgangsart_id" id="qc_vorgangsart_id">

      <div class="qc-photo-section">
        <div class="qc-photo-drop" onclick="this.nextElementSibling.click()">
          <span class="icon">📷</span>
          <div style="font-size:12px; font-weight:800; color:#64748b;">Foto hinzufügen / aufnehmen</div>
        </div>
        <input type="file" name="bilder[]" multiple accept="image/*" style="display:none;" onchange="this.previousElementSibling.querySelector('div').innerText = this.files.length + ' Foto(s) gewählt'">
      </div>

      <div class="qc-field">
        <label>Kategorie / Typ</label>
        <div class="qc-type-selector" id="qc_type_selector">
          <!-- Will be populated by updateProjectContext if project is known -->
          <?php 
          // Initial load for current project context if any
          $pRole = current_role();
          $uId = (int)current_user_id();
          $uBType = 'standard';
          $uRes = $mysqli->query("SELECT business_type FROM benutzer WHERE id=$uId");
          ...
          $qArten = "
              SELECT DISTINCT a.id, a.name, a.icon, a.allowed_roles, a.allowed_business_types 
              FROM pendenzen_arten a
              LEFT JOIN pendenzen_arten_projekte ap ON ap.vorgangsart_id = a.id
              WHERE a.is_active = 1
              AND (
                  ap.projekt_id IS NULL 
                  OR (ap.projekt_id = ".($pidInit ?: '0')." AND (ap.objekt_id IS NULL OR ap.objekt_id = ".($oidInit ?: '0')."))
              )
              ORDER BY a.sort_order, a.name
          ";
          $rsA = $mysqli->query($qArten);
          if($rsA) while($a=$rsA->fetch_assoc()): 
            $isAdmin = in_array($pRole, ['admin', 'superadmin']);
            $roleOk = $isAdmin || empty($a['allowed_roles']);
            if (!$roleOk) {
                $allowed = explode(',', $a['allowed_roles']);
                if (in_array($pRole, $allowed)) $roleOk = true;
            }
            $typeOk = $isAdmin || empty($a['allowed_business_types']);
            if (!$typeOk) {
                $allowed = explode(',', $a['allowed_business_types']);
                if (in_array($uBType, $allowed)) $typeOk = true;
            }
            if (!$roleOk || !$typeOk) continue;
          ?>
            <div class="qc-type-btn" onclick="setQcType(<?= (int)$a['id'] ?>, this)" data-allowed-types="<?= h($a['allowed_business_types']??'') ?>">
              <span class="emoji"><?= h($a['icon'] ?: '📝') ?></span>
              <label><?= h($a['name']) ?></label>
            </div>
          <?php endwhile; ?>
        </div>
      </div>
      ...
      <div class="qc-field">
        <label>Zuständiger Unternehmer</label>
        <select name="zustaendig_id" id="qc_assignee_user_id" class="qc-input">
          <option value="">— Wer ist verantwortlich? —</option>
          <?php if($pidInit): $mem=fetch_members_by_project($mysqli,$pidInit); if($mem) while($m=$mem->fetch_assoc()): 
             $mBkpIds = !empty($m['bkp_ids_comp']) ? $m['bkp_ids_comp'] : ($m['bkp_ids_user'] ?? '');
          ?>
             <option value="<?= (int)$m['id'] ?>" data-bkp="<?= h($mBkpIds) ?>"><?= h($m['name']) ?></option>
          <?php endwhile; endif; ?>
        </select>
      </div>

      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
         <div class="qc-field" style="margin-bottom:0;">
            <label for="qc_bkp_cat_id">Gewerk</label>
            <select id="qc_bkp_cat_id" name="bkp_cat_id" class="qc-input" onchange="loadBkpTemplates(this.value, 'qc')" style="padding: 10px 12px; font-size: 13px;">
              <option value="">— wählen —</option>
            </select>
         </div>
         <div class="qc-field" style="margin-bottom:0;">
            <label for="qc_bkp_tpl_id">Vorlage</label>
            <select id="qc_bkp_tpl_id" name="bkp_tpl_id" class="qc-input" onchange="applyBkpTemplate(this, 'qc')" style="padding: 10px 12px; font-size: 13px;">
              <option value="">— wählen —</option>
            </select>
         </div>
      </div>
      ...
      <div class="qc-field">
        ...
        <input type="text" name="titel" class="qc-input" placeholder="Was ist das Problem?" required>
      </div>

      <div class="qc-context-info" id="qcContextText">
        📍 Projekt > Ort
      </div>

      <button type="submit" class="qc-btn-save">
        🚀 SICHERN
      </button>
    </form>
  </div>
