...
// FIX: Wir nutzen eine Konstante, damit sie ÜBERALL (auch in 1500 Zeilen weiter unten) verfügbar ist
    ...
    foreach ($configs as $field => $cfg) {
        ...
        foreach ($names as $i => $origName) {
            ...
            if ($validUploaderId !== null) {
                $stmt = $db->prepare('INSERT INTO pendenz_dateien (pendenz_id, typ, pfad, mimetype, groesse, titel, is_cover, sort_index, hochgeladen_von) VALUES (?, ?, ?, ?, ?, ?, 0, NULL, ?)');
                if ($stmt) {
                    $typ = $cfg['typ'];
                    $stmt->bind_param('isssisi', $pendenzId, $typ, $targetPublic, $mime, $size, $origName, $validUploaderId);
                    $stmt->execute();
                    $stmt->close();
                }
            } else {
                $stmt = $db->prepare('INSERT INTO pendenz_dateien (pendenz_id, typ, pfad, mimetype, groesse, titel, is_cover, sort_index, hochgeladen_von) VALUES (?, ?, ?, ?, ?, ?, 0, NULL, NULL)');
                if ($stmt) {
                    $typ = $cfg['typ'];
                    $stmt->bind_param('isssis', $pendenzId, $typ, $targetPublic, $mime, $size, $origName);
                    $stmt->execute();
                    $stmt->close();
                }
            }
...
<div class="pneu-wrap<?php echo IS_EMBEDDED_FORM ? ' is-embedded' : ''; ?>">
    <div class="pneu-head">
        <div>
            <h1>Neue Pendenz</h1>
            <div class="pneu-note"><?php echo IS_EMBEDDED_FORM ? 'Direkt eingebunden in die Pendenzenliste.' : 'Separates Formular. Tabelle und Dashboard bleiben unberührt.'; ?></div>
        </div>
...
<?php if (!IS_EMBEDDED_FORM) { require_once __DIR__ . '/../includes/footer.php'; } ?>
