...
<div class="container" style="max-width:1400px;">
  ...
  <header style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:30px; margin-top:20px; flex-wrap:wrap; gap:20px;">
    <div>
      ...
      <?php if ($pid > 0): 
          $curPName = $mysqli->query("SELECT name FROM projekte WHERE id=$pid")->fetch_column();
          $curOName = $oid > 0 ? $mysqli->query("SELECT name FROM objekte WHERE id=$oid")->fetch_column() : null;
      ?>
        <div style="font-size: 15px; font-weight: 700; color: #64748b; margin-top: 5px; display:flex; align-items:center; gap:8px;">
            <span style="background:#f1f5f9; padding:2px 8px; border-radius:6px;">📂 <?= h($curPName) ?></span>
            <?php if($curOName): ?>
                <span style="color:#d1d5db;">›</span>
                <span style="background:#eff6ff; color:#3b82f6; padding:2px 8px; border-radius:6px; display:flex; align-items:center; gap:6px;">
                    🏢 <?= h($curOName) ?>
                    <a href="objekt_edit.php?id=<?= $oid ?>" title="Objekt umbenennen / bearbeiten" style="text-decoration:none; font-size:12px; filter:grayscale(1); opacity:0.6;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.6">✏️</a>
                </span>
            <?php endif; ?>
        </div>
      <?php endif; ?>
    ...
    <div style="display:flex; gap:10px;">
      ...
      <?php endif; ?>
      <?php if ($oid > 0): ?>
        <a href="objekt_edit.php?id=<?= $oid ?>" class="btn secondary">✏️ Objekt bearbeiten</a>
        <a href="wohnung_neu.php?objekt_id=<?= $oid ?>" class="btn primary">➕ Neue Wohnung anlegen</a>
      <?php elseif ($pid > 0): ?>
        <a href="objekt_neu.php?projekt_id=<?= $pid ?>" class="btn primary">➕ Neues Objekt anlegen</a>
      <?php endif; ?>
    </div>
  </header>
    ...
    <div class="unit-grid">
      ...
      ?>
        <div class="unit-card">
          ...
          <div class="unit-footer" style="flex-wrap: wrap;">
             <a href="wohnung_detail.php?id=<?= $wId ?>" class="btn btn-small" style="flex:1; min-width:80px;">🔍 Dashboard</a>
             <a href="wohnung_edit.php?id=<?= $wId ?>" class="btn btn-small secondary" style="flex:1; min-width:80px;">✏️ Edit</a>
             <a href="wohnung_edit.php?id=<?= $wId ?>#rooms" class="btn btn-small outline" style="flex:1; min-width:80px;">🚪 Räume</a>
             <a href="pendenzen.php?projekt_id=<?= $wProj ?>&wohnung_id=<?= $wId ?>" class="btn btn-small secondary" style="flex:1; min-width:80px; background:#f1f5f9; border-color:#e2e8f0; color:#475569;">📋 Tasks</a>
          </div>
        </div>
      <?php endwhile; ?>
    </div>
