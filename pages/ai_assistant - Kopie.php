...
<style>
/* No-Scroll Logic */
body, html { overflow: hidden !important; height: 100%; margin: 0; padding: 0; }
#ai-center-root { background: #ffffff !important; height: 100vh; overflow: hidden; }

/* Premium AI Command Center Design */
.main-content.ai-triple-view { 
    padding: 15px; 
    height: calc(100vh - 85px); /* Angepasst an Header */
...
.message.ai, .message.assistant { 
    background: #ffffff; /* REINWEISSE SPRECHBLASE */
    color: #1e293b !important; /* Dunkle Schrift für perfekten Kontrast */
...
/* Input Area GANZ UNTEN - Höher gesetzt für bessere Ergonomie */
.chat-input-area { 
    padding: 20px 35px 60px 35px; /* Noch höheres Padding unten für 'Floating' Effekt */
    background: linear-gradient(to top, #0b1220 90%, transparent);
...
function newChat() {
    ...
    window.history.pushState({}, '', 'ai_assistant.php');
}
...
async function loadChat(id, event) {
    ...
    try {
        const res = await fetch('../api/ai_chat_messages.php?chat_id=' + id);
...
async function handleInput() {
    ...
    try {
        console.log('Sende Anfrage an gimi...', {text, currentChatId});
        const res = await fetch('../api/ai_query.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ prompt: text, chat_id: currentChatId, context: { url: window.location.href, title: document.title } })
        });
        ...
        if (data.success) {
            if (currentChatId === 0) {
                ...
                window.history.pushState({chat_id: currentChatId}, '', 'ai_assistant.php?chat_id=' + currentChatId);
                
                const chatList = document.getElementById('chat-list');
                const emptyMsg = chatList ? chatList.querySelector('.muted') : null;
                if (emptyMsg) emptyMsg.remove();
                
                const newItem = document.createElement('div');
                newItem.className = 'chat-item active';
                newItem.id = 'chat-item-' + currentChatId;
                newItem.innerHTML = `
                    <div class="chat-click-area" onclick="loadChat(${currentChatId}, event)">
                        <span class="chat-title">${text.substring(0,30)}...</span>
            ...
            errDiv.innerHTML = '<strong>Fehler:</strong> ' + data.error;
        ...
        errDiv.className = 'message ai';
        errDiv.style.color = '#ef4444';
        errDiv.innerHTML = '<strong>Verbindungsfehler:</strong> gimi ist gerade nicht erreichbar.';
        chatMsgs.appendChild(errDiv);
    }
}

input.addEventListener('keypress', (e) => { if (e.key === 'Enter') handleInput(); });
function showAddTraining() { document.getElementById('add-training-form').style.display = 'block'; }
function hideAddTraining() { document.getElementById('add-training-form').style.display = 'none'; }
