...
function _sanitize_redirect_target(?string $url): ?string
{
    ...
    if (strpos($url, $base . '/') !== 0)
        return null;         // muss in der App liegen
    if (strpos($url, $base . '/api/') === 0)
        return null;     // keine API-Endpoints
    return $url;
}
...
{
    $realRole = $_SESSION['rolle'] ?? 'gast';
    if ($realRole === 'superadmin' && !empty($_SESSION['simulate_user_id']))
        return (int) $_SESSION['simulate_user_id'];
    return $_SESSION['user_id'] ?? null;
}
function current_role()
{
    $realRole = $_SESSION['rolle'] ?? 'gast';
    if ($realRole === 'superadmin' && !empty($_SESSION['simulate_role']))
        return $_SESSION['simulate_role'];
...
function is_admin()
{
    return in_array(current_role(), ['admin', 'superadmin'], true);
}
function is_logged_in(): bool
{
    // Im Simulationsmodus (Superadmin simuliert was anderes) gilt man als "logged in"
    if (isset($_SESSION['rolle']) && $_SESSION['rolle'] === 'superadmin' && !empty($_SESSION['simulate_role']))
        return true;
...
if (!function_exists('route_after_login')) {
    function route_after_login(string $rolle): string
    {
        ...
        switch ($rolle) {
            case 'superadmin':
                return $base . "/index_superadmin.php";
            case 'admin':
                return $base . "/index_admin.php";
            case 'benutzer':
                return $base . "/index_private.php";
            default:
                return $base . "/index_public.php";
        }
    }
}
...
function log_user_event(mysqli $db, int $user_id, string $type, array $meta = []): void
{
    ...
    try {
        ...
        // Ignorieren, um Login nicht abzubrechen
    }
}
function update_login_timestamps(mysqli $db, int $user_id): void
{
    $uid = (int) $user_id;
    try {
        // Vermeide Prepared Statements hier, um "Prepared statement needs to be re-prepared" Fehler zu umgehen
        $res = $db->query("UPDATE benutzer SET last_login_at=NOW(), first_login_at=IFNULL(first_login_at, NOW()) WHERE id=$uid");
        if (!$res) {
            $db->query("UPDATE benutzer SET letzter_login=NOW() WHERE id=$uid");
        }
    } catch (\Throwable $t) {
        try {
            $db->query("UPDATE benutzer SET letzter_login=NOW() WHERE id=$uid");
...
function require_project_access(int $projectId): void
{
    global $mysqli;
    $uid = current_user_id();
    if (is_admin())
        return;
    if (!is_project_member($mysqli, $projectId, $uid)) {
        http_response_code(403);
        die("Zugriff verweigert: Sie sind kein Mitglied dieses Projekts.");
    }
}
function can_view_pendenz(mysqli $db, array $p, int $user_id): bool
{
    ...
    if ($user_id <= 0)
        return false; // Gäste haben keine internen Rollen
    ...
    if ($uInfo['business_type'] === 'handwerker') {
        ...
        if ($up) {
            ...
            if ($up['bkp_id'] && (int) ($p['bkp_id'] ?? 0) !== (int) $up['bkp_id'])
                return false;
            return true;
        }
    ...
    // Standard-Logik
    if ((int) ($p['erstellt_von'] ?? 0) === $user_id)
        return true;
    if ((int) ($p['zustaendig_id'] ?? 0) === $user_id)
        return true;

    $sicht = $p['sichtbarkeit'] ?? 'projekt';
    if ($sicht === 'projekt' && is_project_member($db, (int) $p['projekt_id'], $user_id))
        return true;
...
function can_edit_pendenz(mysqli $db, array $p, int $user_id): bool
{
    ...
    if ((int) ($p['erstellt_von'] ?? 0) === $user_id)
        return true;
    if ((int) ($p['zustaendig_id'] ?? 0) === $user_id && (int) ($p['assignee_can_edit'] ?? 1) === 1)
        return true;
