...
.main-nav-container {
  background: var(--nav-bg); color: #fff; z-index: 1000;
...
.nav-brand { padding: 20px; display: flex; align-items: center; gap: 10px; text-decoration: none; color: #fff; }
.nav-brand img { width: 30px; height: 30px; border-radius: 8px; }
.nav-brand span { font-weight: 800; font-size: 17px; }

.nav-menu { list-style: none; padding: 0; margin: 0; display: flex; }
.is-side .nav-menu { flex-direction: column; padding: 0 12px; gap: 4px; }
.is-top .nav-menu { flex-direction: row; margin-left: 20px; gap: 6px; }
...
.nav-menu li.active > a { background: var(--nav-accent); color: #fff; box-shadow: 0 4px 12px var(--nav-accent-glow); }

.nav-utils { margin-left: auto; display: flex; align-items: center; gap: 10px; }
.ai-toggle-small { 
  width: 34px; height: 34px; border-radius: 8px; border: 1px solid var(--glass-border);
  display: flex; align-items: center; justify-content: center; cursor: pointer; background: transparent; color: #fff;
}
.ai-toggle-small.active { background: var(--nav-accent); border-color: var(--nav-accent); }
...
@media (max-width: 1024px) {
  ...
  .is-top .nav-brand { height: 56px; padding:0 15px!important; gap: 8px; }
...
<div id="mainNav" class="main-nav-container <?= $nav_mode === 'top' ? 'is-top' : 'is-side' ?>">
  <a href="<?= e(url('index_private.php')) ?>" class="nav-brand">
    <img src="<?= e(brand_url('logo-mark.png')) ?>" alt="Logo">
    <span>pendenz.com</span>
  </a>

  <ul class="nav-menu">
    <li class="<?= $is_active('index_private.php') ?>"><a href="<?= e(url('index_private.php')) ?>">🏠 Start</a></li>
    <li class="<?= $is_active('pendenzen.php') ?>"><a href="<?= e(url('pages/pendenzen.php')) ?>">📜 Pendenzen</a></li>
    <li class="<?= $is_active('terminprogramm.php') ?>"><a href="<?= e(url('pages/terminprogramm.php')) ?>">📅 Terminprogramm</a></li>
    <li class="<?= $is_active('mieter_dashboard.php') ?>"><a href="<?= e(url('pages/mieter_dashboard.php')) ?>">🏢 Mein Portal</a></li>
    <li class="<?= $is_active('benutzer.php') ?>"><a href="<?= e(url('pages/benutzer.php?view='.$sid)) ?>">👤 Mein Profil</a></li>
    <li class="<?= $is_active('chat.php') ?>"><a href="<?= e(url('pages/chat.php')) ?>">💬 Chat</a></li>
  </ul>

  <div class="nav-utils" style="margin-top:auto; padding: 20px; display:flex; flex-direction:column; gap:10px;">
    <button class="ai-toggle-small" id="ai-toggle-btn" onclick="toggleAiAssistant()" title="Ask gimi">🤖</button>
